...
- 模型调用 `create_goal` / `update_goal` 改变 goal 时，conversation 中出现 typed goal item，明确说明目标已创建、已完成或已更新；该 item 来源于 typed `ResponseItem -> ThreadItem` projection，不从 tool output JSON、assistant 文本或 legacy marker 反解。客户端 goal API 仍以 `thread/goal/updated` / `thread/goal/cleared` 更新当前状态，未来如需要会话流事件也必须复用同一 typed item。
...
- core 在模型工具触发的 `create_thread_goal` / `set_thread_goal` 成功持久化 goal 后，写入 `ResponseItem::ThreadGoalUpdate` 并通过 `record_conversation_items_and_emit_item_completed` 发送 typed lifecycle，同时继续发送现有 `thread/goal/updated` state notification。

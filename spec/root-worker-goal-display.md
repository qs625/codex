...
- `app-server-protocol` 在 shared EventMsg projector 中把 `ThreadGoalUpdateCompleted` 映射为 `ThreadItem::ThreadGoalUpdate`；`ResponseItem::ThreadGoalUpdate` 不再作为 display projector 输入。

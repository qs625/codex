...
- `command_wait` handler 在真正 await 前发 typed `ResponseItemStarted(CommandWait)`，等待返回后用同一个
  item id 记录并发 `ResponseItemCompleted(CommandWait)`。
...
4. handler 生成 stable `ResponseItem::CommandWait` id，并发送 `ResponseItemStarted`。
...
- app-server-protocol：`ResponseItemStarted(CommandWait)` 映射为 v2 `ItemStarted(ThreadItem::CommandWait)`。

...
test("renders client recovery evidence as a typed system card", () => {
  const [entry] = buildConversationEntries(
    makeThread([
      {
        type: "clientRecovery",
        id: "client-recovery:tx-1:req-1",
        recoveryIdentity: "11111111-1111-4111-8111-111111111111",
        launcherClaimId: "22222222-2222-4222-8222-222222222222",
        launcherEvidenceVersion:
          "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        transactionId: "tx-1",
        requestId: "req-1",
        failedBuildId: "build-bad",
        failedBuildHash: "bad-hash",
        sourceCommit: "source-commit",
        requestedByThreadId: "launcher-thread",
        mode: "full",
        failurePhase: "ready-timeout",
        exitCode: 1,
        signal: null,
        readyTimeoutMs: 30_000,
        logPath: "/tmp/recovery.log",
        transactionPath: "/tmp/transaction.json",
        recoveredBuildId: "build-good",
        prompt: "Inspect the rollback evidence.",
  ...
  assert.equal(entry?.toolName, "Runtime recovery evidence");
  assert.match(entry?.text ?? "", /build-good/);
  assert.match(entry?.toolDetails ?? "", /ready-timeout/);
  assert.match(
    entry?.toolDetails ?? "",
  ...
  assert.match(entry?.toolDetails ?? "", /evidence\.json/);
});

test("renders legacy client recovery evidence without inventing a requester", () => {
  const [entry] = buildConversationEntries(
    makeThread([
      {
        type: "clientRecovery",
        id: "client-recovery:legacy:req-1",
        recoveryIdentity: null,
        launcherClaimId: null,
        launcherEvidenceVersion: null,
        ...
        recoveredBuildId: "build-good",
        prompt: "Inspect the legacy rollback evidence.",
        evidencePath: "/tmp/evidence.json",
        recordedAtMs: 42,
      },
    ]),
  ...
  assert.doesNotMatch(entry?.toolDetails ?? "", /Requested by thread/);
  assert.doesNotMatch(entry?.toolDetails ?? "", /Launcher claim/);
  assert.doesNotMatch(entry?.toolDetails ?? "", /Launcher evidence version/);
});

...
test("keeps terminal compact turn text anchors while preserving later visible items", () => {
...
test("keeps process items expanded while a turn is active", () => {
  const state = buildConversationState(
    makeThreadWithTurns([
      {
        id: "turn-active",
...
test("folds terminal turn process items while preserving every typed text message", () => {
  const state = buildConversationState(
    makeThread([
      {
        type: "userMessage",
  ...
  assert.deepEqual(
    state.cells.map((cell) => [
      ...
      cell.entries.map((entry) => entry.messagePhase ?? null),
    ]),
    [
      ["user-1", "message", ["user-1"], [null]],
      ["agent-1", "message", ["agent-1"], ["commentary"]],
      [
        "turn-process:turn-1:cmd-1:cmd-1:1",
        "turnProcess",
        ["cmd-1"],
        [null],
      ],
      ["agent-2", "message", ["agent-2"], [null]],
      [
        "turn-process:turn-1:cmd-2:cmd-2:1",
        "turnProcess",
        ["cmd-2"],
        [null],
      ],
      ["agent-3", "message", ["agent-3"], ["final_answer"]],
    ],
  );
  assert.deepEqual(
    state.cells
      .filter((cell) => cell.kind === "turnProcess")
      .map((cell) => cell.collapsedCells?.map((collapsed) => collapsed.id)),
    [["cmd-1"], ["cmd-2"]],
  );
});

test("does not merge adjacent agent messages with different phases", () => {
  const state = buildConversationState(
    makeThread([
      {
        type: "agentMessage",
  ...
  assert.deepEqual(
    state.cells.map((cell) => [
      cell.id,
      cell.kind,
      cell.entries.map((entry) => entry.messagePhase),
    ]),
    [
      ["agent-commentary", "message", ["commentary"]],
      ["agent-final", "message", ["final_answer"]],
    ],
  );
});

test("folds agent-role file changes instead of treating them as text anchors", () => {
  const state = buildConversationState(
    makeThread([
      {
        type: "agentMessage",
      ...
      },
      {
        ...
        id: "file-1",
        status: "completed",
        changes: [{ path: "/tmp/project/src/app.ts", kind: "modified" }],
      },
      {
        type: "agentMessage",
        id: "agent-2",
  ...
  assert.deepEqual(
    state.cells.map((cell) => [cell.id, cell.kind, cell.entries.map((entry) => entry.id)]),
    [
      ["agent-1", "message", ["agent-1"]],
      ["turn-process:turn-1:file-1:file-1:1", "turnProcess", ["file-1"]],
      ["agent-2", "message", ["agent-2"]],
    ],
  );
});

test("keeps terminal context compaction visible as a top-level compact row", () => {
  const state = buildConversationState(
    makeThread([
      {
        type: "agentMessage",
        id: "agent-before-compact",
        text: "I summarized the earlier process.",
        phase: "commentary",
        memoryCitation: null,
      },
      commandExecutionItem("cmd-1"),
  ...
  assert.deepEqual(
    state.cells.map((cell) => [cell.id, cell.kind]),
    [
      ["agent-before-compact", "message"],
      ["compact-1", "compact"],
      ["agent-after-compact", "message"],
    ],
  );
  assert.equal(state.cells[1]?.entries[0]?.replacementHistoryCount, 1);
});

test("shows terminal failed turn outcome outside folded process items", () => {
  const state = buildConversationState(
    makeThreadWithTurns([
      {
        id: "turn-failed",
        items: [commandExecutionItem("cmd-1")],
        itemsView: "full",
        status: "failed",
        error: { message: "model stream ended" },
        startedAt: 1,
        completedAt: 2,
        durationMs: 1,
      },
  ...
  assert.deepEqual(
    state.cells.map((cell) => [cell.id, cell.kind, cell.entries.map((entry) => entry.text)]),
    [
      [
        "turn-process:turn-failed:cmd-1:cmd-1:1",
        "turnProcess",
        ["tmp/project • exit 0"],
      ],
      ["turn-failed:terminal-outcome", "event", ["Turn failed: model stream ended"]],
    ],
  );
});

test("preserves replacement history message source types and phases", () => {
  const entries = buildConversationEntries(
    makeThread([
      {
        type: "contextCompaction",
  ...
  assert.deepEqual(
    ...
    [
      ["agentMessage", "final_answer", "raw final"],
      ["agentMessage", "commentary", "typed commentary"],
    ],
  );
});

function commandExecutionItem(id: string) {
  return {

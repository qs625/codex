...
- 代码实现完成后，必须先委派独立 `@code-review` 做代码评审；按 review 意见修复到无阻塞问题后，才能委派 `@test_agent` 执行目标测试和回归验证。
...
6. 委派 `@code-review` 执行独立代码评审。
7. 修复 review 发现的问题，直到 reviewer 结论无阻塞问题。
8. 委派 `@test_agent` 执行目标测试和回归验证；owner 不亲自执行测试，只能做非测试性的本地检查、格式化或静态文本验证。
9. 修复测试发现的问题后，回到第 6 步重新 review；再次无阻塞后再委派 `@test_agent` 复测。
10. 按交付格式返回，并分别汇报 reviewer 和 tester 结论。

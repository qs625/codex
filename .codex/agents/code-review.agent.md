...
- reviewer 不能直接执行 Rust/Cargo 相关测试、构建、格式化、lint 或 benchmark 命令，包括 `cargo test/check/build/bench`、`cargo insta`、`just test/fix/fmt`、Bazel Rust lock 验证等。
- review 需要 Rust/Cargo 验证时，必须整理最小命令清单、运行目录、覆盖目标和风险点，委派统一 `@test_agent` 串行执行；review 结论引用 tester 的命令结果、失败摘要和未覆盖范围。
...
4. 执行必要测试或验证，记录命令、结果、失败摘要和未覆盖范围；非 Rust/Cargo 长时间命令遵循 `AGENTS.md` 的订阅与静默等待约束；Rust/Cargo 命令必须委派 `@test_agent` 串行执行，自己不直接运行。

...
- 主 checkout `~/Projects/my-codex` 只作为 PM 集成和合并目录，不承载开发任务，不绑定 owner，也不运行 owner 自测。开发任务只派发到三份固定开发 checkout：`~/Projects/my-codex-dev`、`~/Projects/my-codex-dev-2`、`~/Projects/my-codex-dev-3`。PM 不再为每个任务创建临时开发目录；新任务只能从这三个开发目录中选择空闲目录，已有任务返工复用此前目录。
...
- 合并策略：owner 只在所属开发 checkout 内提交任务分支并交付验证证据，不直接修改或合并主 checkout。PM 在 `~/Projects/my-codex` 当前集成分支执行最终合并、冲突处理和验收记录。合并成功后，空闲开发 checkout 使用 fast-forward 同步到集成分支；非空闲开发 checkout 只记录需要同步的 commit，等 owner 当前任务完成、阻塞暂停或明确同意后再同步。不得用 destructive reset 覆盖未合并工作。
...
9. 明确没问题后由 PM 在主 checkout 合并任务分支，处理冲突，把任务从 Active Work 移到 Completed，并汇报验证证据和剩余风险；随后同步所有空闲开发 checkout，正在开发的 checkout 只记录待同步 commit 和原因。
...
```text
角色：
你是 <checkout> 绑定的固定 owner，负责在该 checkout、分支 <branch> 内串行完成本任务。不要切换到其他 checkout，不要接手依赖未同步的任务。
...
合并职责：
owner 只在所属开发 checkout 提交任务分支并交付验证证据；最终合并到主 checkout、冲突处理和同步其他开发 checkout 由 PM 负责。
...
- PM 确认开发 checkout diff、主 checkout 合并冲突、review 与验证证据、`AGENTS.md` 更新情况、依赖关系和合并顺序。

...
- id: stable-launcher-runtime-update
  ...
  last_update: 2026-09-09 CST minimal rebuild is now approximately 30 tracked files and `+1140/-1135` relative to baseline, versus the rejected 161 files and `+29311/-6191`. Rust Launcher is split across 9 files/~2003 lines with each module under 500 lines; host production JS is ~877 new/rewritten lines; typed recovery is net ~405 lines across its necessary protocol/replay/display touch points. Multi-generation queues/history, legacy migration/repair, fd/inode authority, general journal, public fine-grained checkpoints, GC, and leases are removed. Core contracts now cover Finder no-arg launch, `Root Worker Runtime`, ready identity env, one failure-evidence record, `/self` ensure/resume, and abort-full. Owner identified bundle resource switching as breaking the code-sign seal and is adding only minimal post-switch ad-hoc signing plus strict verification, with rollback and re-sign on failure; review and tests have not started.
  next_action: wait for owner to finish minimal signing/rollback handling, fixed-reviewer review, and focused/full validation; then PM re-check net scope and the full/hot/rollback/typed-recovery matrix before merge
  blockers: none

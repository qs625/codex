...
- id: quarantine-invalid-model-context
  ...
  next_action: fixed dev-2 owner is refactoring source mapping so it is generated by the real outbound compatibility pass and sliced with WebSocket incremental input, then implements immediate non-executed handling, generic structured-item preflight and provider-error durable quarantine; Cargo validation follows implementation, then the fixed reviewer and focused live/reload/compact evidence
  blockers: none

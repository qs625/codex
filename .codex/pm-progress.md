...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST user explicitly promoted framework decoupling and complete Runtime-tree restart into the current implementation. Fixed reviewer has completed two pre-implementation design rounds and still blocks implementation until Launcher-crash races are closed. PM accepted the revised generic fail-stop design: a hidden same-binary guard owns the payload process group and kills it whenever the long-lived Launcher pipe disappears; a restarted Launcher never adopts an old attempt or starts another Runtime until the old guard, PGID and known descendants are proven gone. `run` holds a long-lived executor lock, state mutations use revision/executor-epoch CAS, and commit versus rollback is a mutually exclusive durable decision. Readiness is explicitly a cooperative bearer-token protocol rather than an OS-authenticated writer claim. Seed trust in the current ad-hoc development channel detects corruption/mismatch through bundle verification plus an embedded Seed digest but does not claim production publisher identity. Incoming Capsule handoff is ownership transfer by same-filesystem no-replace rename followed by complete validation; it does not claim protection against a malicious same-uid writer. Outer App replacement rebinds the new embedded Seed with explicit handling for prepared or active old transactions instead of permanently blocking solely because the trusted Seed changed. Normal activation remains strictly stop old complete tree, prove exit, switch selection, start one new Runtime, await matching ready, then commit or rollback; Launcher-level Hot/live activation remains forbidden.
  next_action: fixed dev-3 owner sends the accepted fail-stop guard/executor-lock/CAS/cooperative-ready/Seed-rebind/import-ownership revisions to the same fixed reviewer for a third design review; implementation may begin only after the P0 design review passes
  blockers: pre-implementation fixed-reviewer P0 on crash-safe single-executor/single-Runtime proof; revised design accepted by PM and awaiting third review

...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST fixed reviewer completed a third pre-implementation design round and still blocks implementation. PM accepted the next consolidation with one additional hard invariant: all authoritative selection, generation, activation phase, attempt registration, decision winner, receipt and blocked reason live in one atomically replaced `control.json`; evidence files are display-only. A stable-inode executor lock plus revision/executor-epoch CAS makes one `run` executor authoritative. Recovery first stops and proves exit of the prior guard, payload PGID and known descendants, then obeys the durable winner: rollback/no winner restores fallback, while `CommitDecided` continues candidate commit cleanup and never rolls back. Guard remains outside the payload PGID and both parent-death and guard-death paths trigger payload stop-and-prove. However, the payload entrypoint must not execute until its pid/start identity/PGID registration is durable and recoverable: a child/bootstrap barrier must persist registration before releasing `exec`; merely reporting after successful exec leaves an unregistered simultaneous parent+guard crash window and is not accepted. The formal promise remains fail-stop rather than instant containment: if parent and guard die together, the next Launcher must clear every durably registered attempt before any spawn, and inability to prove exit is `Blocked`. Capsule import uses whole-directory ownership transfer, descriptor-based no-follow validation and a second full verification after `OldStopped`. Legacy migration becomes a replayable per-item move FSM. Canonical digest framing and command-by-phase behavior are fixed as cross-language fixtures. Normal activation remains strictly complete old-tree exit before selection switch/new Runtime execution; there is no Launcher-level Hot/live activation.
  next_action: fixed dev-3 owner incorporates the pre-exec durable payload-registration barrier, makes `CommitDecided` atomically carry the candidate selected-state transition needed for crash recovery, and sends the complete single-control/dual-death-cleanup/commit-winner design to the same fixed reviewer for a fourth review; implementation remains gated on P0 pass
  blockers: pre-implementation fixed-reviewer P0 on eliminating every unregistered payload execution window and proving single-control winner recovery

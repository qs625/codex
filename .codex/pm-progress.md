...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST owner/reviewer containment audit found a platform boundary that requires explicit product choice. Current `setpgid + killpg + process snapshot polling` can prove only observed cooperative processes exited; a Capsule can call `setsid`, double-fork or escape between snapshots. macOS public unprivileged APIs (process groups, kqueue/libproc observation or launchd orchestration) do not provide a non-escapable process container with an authoritative empty proof. Linux can provide the stronger contract through correctly delegated cgroup v2, `cgroup.kill` and recursive `cgroup.events populated=0`. Therefore the protocol must not claim “complete tree proven stopped” on macOS unless scope expands to a separate OS containment primitive; the realistic macOS contract is trusted/cooperative Capsule cleanup with observed-empty evidence and fail-closed behavior when escape or ambiguity is detected. Owner asked PM to choose among three product options, but the completion notification omitted their definitions; PM requested the complete choices before deciding. Previously accepted generic directory symlink, activation/release recovery identifiers, single control state, guard barriers and transaction invariants remain unchanged.
  next_action: owner resends the full 1/2/3 containment choices with macOS/Linux behavior, threat model, scope and recommendation; PM then obtains/records the product decision before implementation continues
  blockers: product decision required on macOS cooperative observed-empty versus strict kernel-contained-empty semantics; option definitions are pending from owner

...
- id: external-versioned-electron-runtime
  ...
  files: macOS package layout and stable Electron Host/preload shim; reloadable external Main Controller contract; Launcher artifact/state/activation/rollback; installed artifact preparation; Electron ready/hot coordination; focused packaging, transaction, controller handoff and real installed-app tests
  base_commit: 08c704430e
  pending_sync_from_main: none; dev-3 fast-forwarded to `08c704430e`
  status: in_progress
  objective: Keep the signed macOS App Bundle stable with Launcher, Electron and a minimal stable Host/preload shim plus signed Seed Runtime; load updateable Main Controller, renderer, app-server and configuration from a versioned external runtime while retaining the same Electron Host process and BrowserWindow across ordinary runtime updates.
  last_update: 2026-09-09 CST user selected a signed read-only Seed Runtime inside the App Bundle and rejected closing the window for ordinary `main.cjs` changes because window acquisition/ownership is stable logic. The required boundary is now a minimal stable Electron Host that permanently owns `BrowserWindow`, view container, scoped IPC/controller registry and runtime handoff; the current product `main.cjs` must be split so updateable business Main logic is a versioned external Controller with explicit activate/snapshot/dispose semantics. A stable preload shim loads the selected external preload/controller on WebContents reload. Ordinary Main/preload/renderer/app-server updates must keep Launcher PID, Electron Host PID and the same BrowserWindow; only Launcher/Electron/stable Host/shim changes may require destructive Full replacement. External current remains preferred, Seed is an immutable final fallback, and source workspace/self-evolution remain optional with no startup clone/build requirement.
  next_action: fixed dev-3 owner implements the stable Host plus reloadable Controller architecture, reuses its fixed reviewer, and returns focused plus real packaged macOS evidence
  blockers: none
  validation: pending; must cover missing/corrupt current, Seed bootstrap, current/previous atomicity, same-Host/same-Window Main Controller handoff, preload/renderer reload, app-server restart, controller activation rollback, Host crash Seed fallback, App Bundle immutability/hash stability, no activation-time codesign, and real packaged macOS launch/update
  commit:

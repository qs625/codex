...
- id: eventmsg-threaditem-projector-boundary
  ...
  last_update: 2026-06-16
  next_action: Done. Adapter implemented for display-capable EventMsg variants; follow-up cleanup removed ResponseItem lifecycle display replay.
...
- id: migrate-wait-command-display-events
  ...
  next_action: Done. Dedicated `CommandWaitStarted`, `CommandWaitCompleted`, `CommandWriteStdinCompleted`, and `CommandExecutionNotificationCompleted` variants are implemented and validated.
  blockers: eventmsg-dual-write-helpers.
  validation: passed via fixed tester requests eventmsg-full-refactor-20260617-02 and eventmsg-full-refactor-20260617-05-rerun-protocol; schema fixtures regenerated by eventmsg-full-refactor-20260617-04-write-schema after fixture mismatch.
  commit: 67509dd
- id: migrate-collab-goal-workflow-display-events
  ...
  last_update: 2026-06-17
  next_action: Done. Dedicated `InterAgentCommunicationCompleted`, `ThreadGoalUpdateCompleted`, `WorkflowRunProgressCompleted`, `EventCommandEventCompleted`, and `EventDrivenToolCompleted` variants are implemented and validated.
  blockers: eventmsg-dual-write-helpers.
  validation: passed via fixed tester requests eventmsg-full-refactor-20260617-02 and eventmsg-full-refactor-20260617-05-rerun-protocol; schema fixtures regenerated by eventmsg-full-refactor-20260617-04-write-schema after fixture mismatch.
  commit: 67509dd
- id: responseitem-contract-tightening
  ...
  last_update: 2026-06-17
  next_action: Done. Primary live/read display paths use dedicated EventMsg variants; `ResponseItemStarted/Completed` and structured display ResponseItems no longer replay display items.
...
- id: legacy-display-path-cleanup
  ...
  branch: feat/tool-callback
  status: completed
  objective: Remove legacy display paths after EventMsg projection covers live/read.
  last_update: 2026-06-17
  next_action: Done. Structured `ResponseItem` projection fallback, `RawResponseItem` display replay, `RolloutItem::ResponseItem` display replay, and public `TurnItem -> ThreadItem` adapter have been removed.
  ...
  validation: passed via fixed tester requests eventmsg-full-refactor-20260617-02 and eventmsg-full-refactor-20260617-05-rerun-protocol; schema fixtures regenerated by eventmsg-full-refactor-20260617-04-write-schema after fixture mismatch.
  commit: 67509dd
- id: remove-legacy-display-compat
  owner: /root/my_codex_pm
  worktree: /Users/bytedance/Projects/my-codex
  branch: feat/tool-callback
  status: completed
  objective: Remove old rollout/history display compatibility paths so app-server v2 ThreadItem display is generated only from display-capable EventMsg, not ResponseItem, RawResponseItem, or public TurnItem adapters.
  last_update: 2026-06-17
  next_action: Commit cleanup.
  blockers: None.
  validation: fixed tester request remove-legacy-display-compat-20260617-08-protocol-rerun passed `rtk cargo test -p codex-app-server-protocol`; fixed tester request remove-legacy-display-compat-20260617-09-app-server-build passed `rtk cargo build -p codex-app-server --bin codex-app-server`; local `rtk git diff --check` passed.

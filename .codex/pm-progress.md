...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST PM selected containment option 1 after receiving the complete decision package. Runtime Capsule v1 is explicitly a trusted/cooperative supervised-runtime contract, not an adversarial same-UID sandbox. On macOS, guard + dedicated PGID + pid/start-identity tracking + bounded TERM/KILL + residual observation may produce only typed `CooperativeObservedEmpty` evidence; it must never be named or serialized as kernel-proven complete-tree emptiness. Manifest/producer contract forbids daemonize, double-fork, `setsid` and process-group escape. Any observed escape, identity mismatch, cleanup timeout, ambiguous process ownership or residual process makes the durable state `Blocked` before selection switch, fallback restoration, commit or new spawn. This preserves macOS Capsule activation for trusted Morpheus-built payloads while honestly excluding never-observed malicious escapees from the guarantee. Linux uses the same cooperative backend in v1; cgroup v2 `cgroup.kill` plus `populated=0` may be added later as a distinct stronger proof level. Strict kernel-contained activation disabling macOS and a privileged macOS helper/system-extension project are rejected for the current scope. Previously accepted generic directory symlink, activation/release recovery identifiers, single control state, guard barriers and transaction invariants remain unchanged.
  next_action: fixed dev-3 owner resumes implementation under the selected cooperative containment contract, updates proof names/manifest/evidence/tests and stable architecture wording, completes the remaining reviewer findings, then returns to the same fixed reviewer and runs focused plus real packaged Electron validation
...
- id: quarantine-invalid-model-context
  ...
  pending_sync_from_main: PM-only progress commits through `33b7a2922c`; dev-2 completed from product baseline `483d5bfbe2`, so PM will merge the submitted commit before synchronizing the checkout
  status: review
  ...
  last_update: 2026-09-10 CST owner delivered commit `5126f6a5ea` after multi-round fixed review, but PM design acceptance found a direct P0 against the reproduced provider contract. `model-service/src/responses_requests.rs` defines `MAX_STRUCTURED_JSON_PROPERTY_NAME_BYTES = 1_024`, while the actual incident error states a maximum property-name length of 256. The code also measures UTF-8 bytes although the provider reports string length, and the provider's real error code from this incident is `property_name_above_max_length`; `is_recoverable_invalid_model_input_code` does not allow that code. Consequently a structured property name between 257 and 1024 characters can bypass local preflight, be rejected by the provider, and then fail the quarantine allowlist, preserving the original thread-poisoning class. The broader architecture remains sound: typed durable markers, raw evidence preservation, projected context, exact compatibility/WebSocket source mapping, complete unique transaction quarantine, shared ordinary/compact recovery, local non-executed handling and token baseline recomputation are present. Canonical project-understanding additions are acceptable at the architecture level.
  next_action: fixed dev-2 owner corrects the known provider property-name boundary and units, recognizes the exact observed `property_name_above_max_length` code, adds 256/257 plus multibyte boundary and real-error-envelope end-to-end recovery tests, amends the task commit, and returns to the same fixed reviewer; PM then repeats design acceptance before merge
  blockers: PM P0: local preflight and provider recovery allowlist do not match the exact 256-length / `property_name_above_max_length` incident contract
  validation: fixed reviewer passed with no blocking findings after token-boundary, local-malformed Completed-usage timing and rustfmt-churn corrections. Owner: context-manager 115 passed; context-usage 10 passed; thread-service quarantine 8 passed; local malformed token-baseline regression 1 passed; model-service clients 9 passed; app-server production build passed; git diff check passed. Full model-service/model-service-api unit targets remain blocked by pre-existing missing test source modules. Residual non-blocking suggestions cover a second non-zero usage response, same-response assistant output after malformed call and no-Completed interruption paths
  commit: 5126f6a5ea989d3b3255377b5bc8433e3bbc94f5

...
- id: quarantine-invalid-model-context
  ...
  last_update: 2026-09-09 CST PM reproduced the complete failure chain. A malformed client `tool_search_call` stored 1,872 characters of assistant scratch text as an `arguments` object property name. Its `status: completed` came from the provider output-item lifecycle and means only that the model finished emitting the call, not that the client tool executed successfully. `ToolCall::from_response_item` then failed to deserialize `SearchToolCallParams`; the `RespondToModel` branch nevertheless persisted the raw malformed call, emitted a generic `FunctionCallOutput` with an empty call id, and requested model follow-up. That recovery request replayed the same invalid call, so the provider rejected the whole request before streaming with `property_name_above_max_length`; rollout/compact then made every later turn repeat the failure. Manual compact-head repair restored the thread. The fix must therefore cover both new failures and legacy poison: a pre-dispatch parse/schema failure must be recorded as typed failed/non-executed evidence without replaying the raw invalid arguments as a provider tool call, must preserve the original call id and correct tool-output pairing where the protocol permits, and must tell the model through a bounded sanitized recovery item. The generic durable `input[n]` quarantine/retry path remains required for already persisted invalid items and provider-side constraints that local validation cannot predict. Current user input, system/developer instructions and items that cannot be mapped uniquely must fail closed rather than be silently deleted.

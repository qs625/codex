...
- id: quarantine-invalid-model-context
  ...
  last_update: 2026-09-09 CST fixed reviewer found two additional model-input consumers/invalid-call shapes bypassing recovery. First, the `ContextWindowExceeded` suffix-staged compact path still terminates directly on `InvalidModelInput` instead of using the shared helper and per-turn cap. Second, a client `ToolSearchCall` with `call_id=None` currently returns `Ok(None)` from `ToolCall::from_response_item`, incorrectly treating a malformed tool call as a normal non-tool item and bypassing the typed non-executed marker. Owner is routing ordinary sampling, pre-turn inline compact, context-window suffix-staged compact and standalone/manual compact through one `quarantine_recoverable_model_context` helper. Each top-level turn/task owns its bounded set; compact internals must not create a separate budget or approximate recovery loop. A missing-call-id tool search is an invalid structured tool item: it must be quarantined by a stable provider item identity or canonical fingerprint when uniquely identifiable, recorded as non-executed, and reported through a sanitized notice without fabricating a call id or paired success output. Retained-suffix marker reconciliation remains fixed.
  next_action: owner closes suffix-staged and standalone/manual compact recovery gaps, handles `ToolSearchCall(call_id=None)` as an invalid non-executed structured item, adds shared-cap and all-consumer regression coverage, then returns to the same fixed reviewer; focused Cargo validation runs only after review acceptance. PM merge acceptance must inspect `.morpheus/instructions/project-understanding.md`
  blockers: fixed-reviewer P0: context-window suffix-staged compact and missing-call-id client tool search still bypass the shared quarantine recovery contract

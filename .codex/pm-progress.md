...
- id: stable-launcher-runtime-update
  ...
  last_update: 2026-09-09 CST minimal rebuild is approximately 30 tracked files and `+1140/-1135` relative to baseline, versus the rejected 161 files and `+29311/-6191`. First review correctly blocked testing on remaining signing/candidate atomicity, Host manifest ordering, and typed RPC trust/idempotency issues. The corrected design now keeps current/previous/candidate under the state root; bundle-local swap paths are temporary and removed before top-level signing plus deep verification; rollback reconstructs from state-root previous rather than retaining a long-lived Contents backup. Host signs the nested app-server before manifest generation. Typed recovery RPC is bound to the registered Host connection, server-validates system `/self`, and uses complete history plus a scoped mutex for idempotency. Fixed reviewer is still re-reviewing; no validation or commit is accepted yet.
  next_action: wait for fixed reviewer approval and owner validation/commit; then re-check net scope, state-root atomicity, signing rollback, manifest/hash ordering, RPC trust/idempotency, core full/hot/rollback/typed-recovery matrix, and final evidence before merge
  blockers: none

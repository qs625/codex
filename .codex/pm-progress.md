...
- id: quarantine-invalid-model-context
  ...
  last_update: 2026-09-09 CST fixed reviewer found two blocking recovery gaps. Owner fixed compact retained-suffix reconstruction so an isolated quarantine marker is reconciled by `reconcile_model_context_quarantines`, with marker-only coverage. The remaining P0 is pre-sampling inline compact: it invokes model sampling before the main `run_turn` quarantine loop, so a structured `InvalidModelInput` can repeatedly replay poisoned history, consume stream retries and terminate without quarantine recovery. Owner is moving inline compact sampling onto the same strict real-outbound source mapping, complete uniquely identified tool-transaction quarantine and per-turn bounded quarantine state machine as ordinary sampling. Transient transport retries must not consume the quarantine budget, while distinct durable quarantines do. Pre-turn compact and poisoned transaction regression coverage is required. Current user input, system/developer instructions and items without a unique real-sent mapping remain protected/fail-closed.
  next_action: owner fixes pre-sampling inline compact to reuse the shared quarantine state machine, adds poisoned-transaction/pre-turn-compact tests, then returns to the same fixed reviewer; focused Cargo validation runs only after reviewer accepts the design and implementation shape. PM merge acceptance must also inspect `.morpheus/instructions/project-understanding.md` for conflicts, duplication and overly specific implementation wording
  blockers: fixed-reviewer P0: inline compact sampling currently bypasses the main invalid-model-input quarantine recovery loop

...
- id: quarantine-invalid-model-context
  ...
  last_update: 2026-09-10 CST owner delivered follow-up commit `5c84cf9f14` and the same fixed reviewer passed it. The local structured-output preflight now uses a 256 maximum and the exact observed code `property_name_above_max_length`; recovery allowlisting recognizes that code. Because the provider has not documented whether non-ASCII length is Unicode scalar count or UTF-8 bytes, the implementation conservatively rejects when either exceeds 256, ensuring it is not wider than the known provider boundary. Added coverage proves ASCII 256 passes while 257, 512 and 1024 are rejected before transport; multibyte values exercise conservative byte/scalar boundaries; real SSE and WebSocket invalid-request envelopes remain mapped to the exact transaction after compatibility filtering; and both provider-fallback and local-preflight paths durably quarantine call+output, emit a bounded notice and retry cleanly in the same turn. The broader first commit retains typed durable markers, raw evidence, projected context/token accounting, actual outbound/WS suffix mapping, all sampling/compact consumers and local non-executed handling. PM is repeating final diff/design acceptance before merge.
  next_action: PM inspects `5c84cf9f14` and the combined branch against the P0 contract; if accepted, merge both commits through Git into main, rerun focused integration/build validation, review canonical project-understanding wording, update progress and synchronize idle dev-2
  blockers: none
  validation: fixed reviewer passed both commits. Owner aggregate: context-manager 115 passed; context-usage 10 passed; thread-service quarantine 8 passed; local malformed token baseline 1 passed; model-service clients 9 passed; SSE end-to-end 5 passed; WebSocket idle-timeout/integration 4 passed; property-name thread-service tests 2 passed; app-server production build and git diff check passed. Full model-service/model-service-api unit targets remain blocked by pre-existing missing test modules
  commit: 5126f6a5ea989d3b3255377b5bc8433e3bbc94f5, 5c84cf9f14

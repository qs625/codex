...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST fourth fixed-reviewer design round reduced the remaining P0s to pre-registration ordering and rollback stop proof. Owner selected the stronger closure for a fifth review: guard must durably self-register and receive Launcher acknowledgement before it may fork a payload; after fork, the child must still remain behind the previously required barrier until payload pid/start identity/PGID registration is durable before entrypoint exec. Rollback is an explicit `RollbackDecided -> StoppingCandidate -> CandidateStopped -> Restoring` sequence, and fallback launch must reference the stop proof produced by that exact candidate attempt. `CommitDecided` recovery never changes the winner: it first cold-launches and obtains ready from the already committed selected Runtime, then performs cleanup that cannot affect selected state; launch failure follows committed-current crash handling rather than converting to rollback. Single atomic `control.json`, executor lease/epoch CAS, guard/payload dual-death cleanup, complete old-tree exit before activation, cooperative readiness, Seed rebind, owned incoming handoff,/payload revalidation, replayable legacy migration and cross-language digest fixtures remain required. No product implementation has begun while the design P0 gate remains open.
  next_action: fixed dev-3 owner completes the fifth design review with the same fixed reviewer; review must verify guard acknowledgement precedes fork, forked child cannot exec before durable payload registration, fallback references the exact `CandidateStopped` proof, and `CommitDecided` recovery cannot mutate the winner
...
- id: quarantine-invalid-model-context
  ...
  pending_sync_from_main: PM-only progress commits through `313e5604e8`; dev-2 is actively developing from product baseline `483d5bfbe2` and does not need synchronization
  status: in_progress
  ...
  last_update: 2026-09-09 CST PM reproduced the complete failure chain. A malformed client `tool_search_call` stored 1,872 characters of assistant scratch text as an `arguments` object property name. Its `status: completed` came from the provider output-item lifecycle and means only that the model finished emitting the call, not that the client tool executed successfully. `ToolCall::from_response_item` then failed to deserialize `SearchToolCallParams`; the `RespondToModel` branch nevertheless persisted the raw malformed call, emitted a generic `FunctionCallOutput` with an empty call id, and requested model follow-up. That recovery request replayed the same invalid call, so the provider rejected the whole request before streaming with `property_name_above_max_length`; rollout/compact then made every later turn repeat the failure. Manual compact-head repair restored the thread. The fix covers both new failures and legacy poison: a pre-dispatch parse/schema failure becomes typed failed/non-executed evidence plus a sanitized correction item without replaying raw malformed arguments, while provider invalid-input errors use durable exact-item quarantine. Owner's read-only implementation audit identified an additional mapping invariant: Responses WebSocket may use `previous_response_id` and send only an incremental input suffix, so provider `input[N]` indexes that actual suffix rather than the complete prompt. Source mapping must therefore be produced by the same compatibility/filtering pass as the real outbound request and undergo the identical WebSocket delta slicing; a separately reconstructed full-prompt index map is forbidden. Current user input, system/developer instructions and items that cannot be mapped uniquely must fail closed rather than be silently deleted.
  next_action: fixed dev-2 owner implements the accepted layered recovery: immediate typed non-executed handling for locally invalid tool calls; a generic preflight over every provider-bound structured model-output item so oversized/invalid JSON keys are quarantined before replay rather than only special-casing `tool_search`; and precise real-outbound-index durable quarantine for legacy or provider-only invalid inputs. Plain assistant text that happens to contain JSON remains ordinary text. Owner then reuses `/self/owner_dev_2/reviewer` and returns focused live/reload/compact/provider-error evidence
  blockers: none
  validation: pending; must cover the exact oversized tool-search property reproduction, oversized property names in other provider-bound structured model outputs, the same JSON rendered only as plain assistant text, structured `input[n]` mapping after compatibility filtering and WebSocket suffixing, paired call/output quarantine, durable reload and compact behavior, sanitized model notification without replaying toxic raw arguments, successful bounded retry, multiple invalid items within cap, cap exhaustion, malformed/missing/out-of-range provider param, current-user/system/developer item protection, non-invalid-request errors unchanged, SSE and WebSocket paths, and transparency of original persisted/display evidence
  commit:

...
- id: quarantine-invalid-model-context
  ...
  last_update: 2026-09-10 CST owner delivered commit `5126f6a5ea` after multi-round fixed review. The implementation adds typed durable `ModelContextQuarantine` markers with transaction/item references and canonical fingerprints; preserves raw rollout/display evidence; constructs projected model context by atomically excluding matched poison; keeps context usage, token/byte estimates and compact decisions on the same projection; carries a source sidecar through the exact Responses compatibility transform and WebSocket incremental suffix; and permits structured `InvalidModelInput` recovery only for a unique complete mapping. Ordinary sampling plus pre/mid/suffix/standalone compact share one bounded quarantine contract. Local malformed structured calls are retained as raw evidence but not executed, receive fingerprint quarantine without empty-call-id output, and recompute projected token baselines after provider usage is accumulated once. A generic structured-call outbound preflight catches known oversized recursive JSON property names while plain assistant text is unchanged. `.morpheus/instructions/project-understanding.md` was modified and requires PM canonical merge review.
  next_action: PM inspects commit `5126f6a5ea` against the full design invariants, changed-file scope and canonical project-understanding wording; if accepted, merge via Git into main, rerun focused tests/build, update progress and synchronize the now-idle dev-2 checkout
  blockers: none

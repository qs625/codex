...
- id: generic-runtime-capsule-launcher
  ...
  last_update: 2026-09-09 CST owner has moved into implementation after the crash/recovery design gates and is using the fixed reviewer. A real Electron payload audit found that standard signed Framework layouts require relative symlinks whose final target is a directory, such as `Versions/Current -> A`; the earlier “symlink must end at a regular file” rule cannot represent a complete Electron Capsule. PM accepted the generic correction: every symlink hop and final target must resolve lexically within the Capsule to an explicitly declared manifest entry; the final entry may be a declared directory or regular file, with no absolute/escaping/dangling/cyclic/undeclared/special targets. Directory aliases are not followed to discover content during validation; all real canonical leaves remain separately declared. `launch.executable` remains a directly declared executable regular file and cannot itself be a symlink. The Electron recovery adapter also replaces legacy transaction/build/mode identifiers with generic `activationId`, `releaseId` and optional `fallbackReleaseId`; it must not fabricate a `full` mode now that Launcher activation has one complete-restart semantic. Single atomic `control.json`, executor lease/epoch CAS, guard/payload pre-registration barriers, exact candidate stop proofs, irreversible commit winner, cooperative readiness, Seed rebind, owned incoming handoff/revalidation, replayable legacy migration and cross-language digest fixtures remain required.
  next_action: fixed dev-3 owner sends the directory-symlink contract through the same fixed reviewer, implements Launcher and Electron Capsule producer/recovery adapter consistently, and returns focused plus real signed Electron Framework package evidence
  blockers: none; directory-symlink incremental review pending but the generic contract is decided
...
- id: quarantine-invalid-model-context
  ...
  pending_sync_from_main: PM-only progress commits through `386faf62eb`; dev-2 is actively developing from product baseline `483d5bfbe2` and does not need synchronization
  status: in_progress
  ...
  next_action: fixed reviewer `/self/owner_dev_2/reviewer` reviews the current implementation and stable architecture documentation before owner runs focused Cargo validation; PM merge acceptance must inspect `.morpheus/instructions/project-understanding.md` for conflicts, duplicate facts, over-specific implementation wording and consistency with the canonical model-context/replay architecture
  blockers: none

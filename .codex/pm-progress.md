...
- id: responseitem-contract-tightening
  ...
  last_update: 2026-06-17
  next_action: Done. Primary live/read display paths now use dedicated EventMsg variants while `ResponseItemStarted/Completed` and structured display ResponseItems remain as legacy/model-history compatibility.
...
- id: legacy-display-path-cleanup
  ...
  last_update: 2026-06-17
  next_action: Done. Current diff quarantines structured `ResponseItem` projection as fallback for old rollouts and suppresses only matching fallback when dedicated display EventMsg exists.
...
- `ResponseItem -> ThreadItem` remains only as a legacy compatibility adapter during migration and for model-history fallback, not the primary display path.
...
- commit: `67509dd`
  summary: Dedicated display EventMsg variants landed for command wait/stdin/notification, workflow progress, goal lifecycle, event-command, event-driven tool, and inter-agent display. app-server live/read display now projects these EventMsg variants to ThreadItem directly; structured ResponseItem display projection is quarantined as legacy/model-history fallback with matching fallback suppression. Schema fixtures, AGENTS.md, and EventMsg/ResponseItem specs were updated to reflect EventMsg as the display source and ResponseItem as model context/provider history.
  ...
  residual_risk: Full workspace tests were not run; legacy rollout/history compatibility paths are intentionally retained.
- commit: `1f4e54e`

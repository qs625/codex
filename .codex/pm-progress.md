...
- Step 6C 第十七个切片把 Codex Apps accessible connector discovery/cache 迁到 `codex-mcp-runtime::connectors`：包括 MCP tool -> `AppInfo` 聚合、accessible connector cache key/read/write、Codex Apps MCP startup/ready 等待、force refresh、plugin source enrichment、cached accessible list 和 manager 上的 enabled connector 过滤；core `connectors.rs` 收缩为兼容 re-export facade，并只保留 tool-suggest connector/plugin discoverable 合并这种依赖 core plugin runtime 的聚合逻辑。connector discovery/cache 单测迁到 `codex-mcp-runtime`，core 继续保留 app policy、requirements 和 tool-suggest facade 测试。
- Step 6C 第十八个切片把 thread fork snapshot history transform 迁到 `codex-rollout-api::fork_snapshot`：包括 `ForkSnapshot` public DTO、interrupted fork 的 model-visible marker policy、`<turn_aborted>` fragment、snapshot mid-turn detection、truncate-before-nth-user-message、interrupted boundary append 和 fork history transformation。core `ThreadManager` 只保留 thread store 读取、config -> marker policy 映射、spawn/thread-source adapter 和 `ForkSnapshot` 兼容 re-export；真实 interrupt path 与 interrupted fork snapshot 现在共享 `codex_rollout_api::interrupted_turn_history_marker`。相关纯单测迁到 `codex-rollout-api`，core 继续保留 thread manager 集成测试。
...
- 当前 `codex-rs/core/src` 约 270 个 Rust 文件、107106 行；`core/src/exec.rs` 已从 1517 行降到约 619 行，`core/src/thread_manager.rs` 已降到约 1706 行，`core/src/goals.rs` 约 1329 行，`core/src/session/mod.rs` 已降到约 4014 行，Windows sandbox override 单测迁到 `codex-sandboxing-api`，exec timeout/capture DTO 单测迁到 `codex-command-runtime`，本地 process output capture、sandbox-denial/result interpretation 和 process exec request DTO 迁到 `codex-process-exec`，goal hidden-context prompt 单测、goal runtime state、direct child completion tracker 和 post-turn selector 迁到 `codex-agent-runtime`，accessible connector discovery/cache 迁到 `codex-mcp-runtime`，thread fork snapshot transform 和 interrupted marker policy 迁到 `codex-rollout-api`，unified-exec exec-server env policy 不再污染通用 `ExecRequest`；`core/src/unified_exec` 剩余最大文件为 `process_manager.rs`、`process.rs`、`async_watcher.rs`。
...
- `rtk cargo build -p codex-app-server --bin codex-app-server`：通过，仅既有 warnings。
- `rtk git diff --check`：通过；touched Rust 文件 `rtk rg -n "unsafe"` 无命中，本切片没有新增 unsafe。
- `rtk cargo test -p codex-mcp-runtime connectors -- --nocapture`：通过 3 条，覆盖迁入 owner crate 的 MCP tool -> accessible connector 聚合和 accessible connector cache refresh。
- `rtk proxy cargo test -p codex-core connectors -- --nocapture`：通过，覆盖 core facade 下保留的 app policy、requirements 和 tool-suggest 聚合测试。
- `rtk cargo check -p codex-core --lib`：通过，仅既有 warnings，覆盖 core `connectors.rs` 收缩为 `codex-mcp-runtime` facade 后的生产编译。
- `rtk cargo build -p codex-app-server --bin codex-app-server`：通过，覆盖 app-server/TUI-facing connector API 继续经 core facade 编译。
- `rtk cargo tree -p codex-mcp-runtime --depth 2 --edges normal`：direct graph 新增 `async-channel` 和 `codex-connectors-api` 轻量边界；仍不依赖 core。
- `codex-mcp-runtime` accessible connector 迁移后 normal graph 精确 grep 门禁和 `cargo tree --workspace --edges normal --invert <heavy>` 反向门禁：core、app-server protocol、code-mode、concrete network-proxy、exec-server、state/sqlx、codex-api、concrete `codex-openai-files`、concrete `codex-core-skills` 均 PASS。
- `rtk just bazel-lock-check`：通过，仅既有 rules_rs well-known crate annotation warnings。
- `rtk git diff --check`：通过；touched Rust 文件 `rtk rg -n "unsafe"` 无命中，本切片没有新增 unsafe。
- `rtk cargo test -p codex-rollout-api -- --nocapture`：通过 20 条，覆盖迁入后的 fork snapshot truncation、mid-turn detection、interrupted boundary append、developer/contextual marker policy 和 resumed-history transform。
- `rtk cargo test -p codex-core --lib snapshot -- --nocapture`：通过 41 条（包含 thread manager interrupted fork snapshot 集成覆盖和 shell snapshot 相关过滤命中），确认 `ThreadManager::fork_thread` 继续对 persisted mid-turn history 追加 interrupted boundary，并按真实 config path 写 developer marker。
- `rtk cargo test -p codex-core --lib thread_manager -- --nocapture`：通过，覆盖 thread manager 26 个过滤命中测试。
- `rtk cargo check -p codex-core --lib`：通过，仅既有 warnings，覆盖 core 改用 `codex_rollout_api::fork_history_from_snapshot` 和 shared marker helper 后的生产编译。
- `rtk cargo build -p codex-app-server --bin codex-app-server`：通过，覆盖 app-server 入口继续经 core `ForkSnapshot` 兼容 re-export 编译。
- `rtk cargo tree -p codex-rollout-api --depth 2 --edges normal`：direct graph 仅为 `codex-context-manager` 和 `codex-protocol`，不依赖 core。
- `codex-rollout-api` fork snapshot 迁移后 normal graph 精确 grep 门禁和 `cargo tree --workspace --edges normal --invert <heavy>` 反向门禁：core、app-server protocol、code-mode、concrete network-proxy、exec-server、state/sqlx、codex-api、concrete `codex-openai-files`、concrete `codex-core-skills` 均 PASS。

## Next Action

继续 Step 6C：在 process capture、process result interpretation、process exec request DTO、goal prompt policy、goal runtime state、direct child completion tracker/post-turn selector、accessible connector discovery/cache、thread fork snapshot transform 和 unified-exec env policy 已拆出/收口后，继续评估 `ExecRequest` / Windows sandbox execution glue、unified-exec process manager service 边界、tool orchestrator host trait 或 workflow/thread-store runtime 边界；真实 sandbox selection、EventMsg emission、Session/TurnContext 编排、thread store/read adapter 以及 core-owned plugin/tool-suggest 聚合暂留 core，避免把 heavy runtime 通过 indirect graph 拉回。

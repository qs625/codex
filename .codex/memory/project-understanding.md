...
- live thread status read / subscribe 已提升到 provider-neutral `ThreadLifecycleRuntime`；app-server thread/turn processors 和 thread-service `AgentControl` 不再依赖 `AppServerLiveThreadStatusRuntime` / `LiveThreadStatusRuntime`。read 语义仍是 external live record 优先、否则 native live thread status；subscribe 仍是 native live thread watch 语义。旧 `LiveThreadStatusRuntime` compatibility surface 已删除。
...
- listener generation、running resume response、rollback response 和 permissions request 缺 cwd fallback 所需的 session/config copied reads 已迁到 `LiveThreadInspectionRuntime` / `AppServerLiveThreadInspectionRuntime` by thread id；listener handle 不再暴露 `session_configured` / `config_snapshot`，仍保留 event stream、submit、runtime status 和 read-thread 等后续独立切片能力。

...
- listener generation、running resume response、rollback response 和 permissions request 缺 cwd fallback 所需的 session/config copied reads 已迁到 `LiveThreadInspectionRuntime` / `AppServerLiveThreadInspectionRuntime` by thread id；bespoke approval / elicitation / user-input / permissions responses 和 dynamic tool responses 的 listener submit 已迁到 `LiveThreadCommandRuntime` / `AppServerLiveThreadCommandRuntime` by thread id。listener handle 不再暴露 `session_configured` / `config_snapshot` / submit，仍保留 event stream 和 read-thread 等后续独立切片能力。

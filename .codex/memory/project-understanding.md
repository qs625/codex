...
- `LiveThreadInspectionRuntime` 的 `list_live_thread_ids` / `is_live_thread_loaded` / `live_thread_info` 已补齐 external live record 可见性，和既有 external snapshot/config snapshot 特判保持一致；这不代表 external close/remove、reload/list_agents 或 root start provider routing 已完成。

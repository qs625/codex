...
- 当前 external runtime 仍是 live-only registry 技术债，但不能再用 one-shot/re-invocation 冒充连续 turn；支持的 provider 必须走常驻 stdin/stdout streaming transport，unsupported provider 要显式报 capability error。

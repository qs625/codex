...
- listener generation、running resume response、rollback response 和 permissions request 缺 cwd fallback 所需的 session/config copied reads 已迁到 `LiveThreadInspectionRuntime` / `AppServerLiveThreadInspectionRuntime` by thread id；listener handle 不再暴露 `session_configured` / `config_snapshot`，仍保留 event stream、submit 和 read-thread 等后续独立切片能力。

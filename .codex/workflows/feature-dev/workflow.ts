...
function resultStatus(result) {
  return result?.status ?? result?.event?.communication?.status;
}
...
function waitResultText(result) {
  for (const value of [
    result?.text,
    result?.message,
    statusMessage(resultStatus(result)),
    result?.content,
    result?.event?.communication?.content
  ]) {
...
async function waitForAgentResult(agent, label) {
  ...
  const kind = result?.statusKind ?? statusKind(resultStatus(result));

...
- 记“结论、状态、关键设计理解、文件地图、下一步”，不要记大段代码、长日志或临时推理。
...
后续模型对下面这些问题的判断，必须以这份文件为准，而不是依赖 `AGENTS.md`、旧 compact 摘要或分散在别处的零散规则：

- 这个项目应该怎么工作
- 哪些边界必须遵守
- 哪些模块负责什么
- 某条行为背后的长期设计意图是什么
- 为什么系统采用当前方案，而不是其他看起来也能工作的方案

这意味着 `project-understanding.md` 不能只停留在“目录/文件地图”。如果当前上下文已经足以确认某个重要设计事实，就应该写进去，例如：

- 某条 display/rendering 链路为什么必须走 typed event 而不能回退到 raw marker
- 某类等待/唤醒为什么统一走 `poll_event`
- 某个状态机里什么信号只负责投递，什么信号才真正定义 active / complete
- 某个兼容层为什么保留，哪些地方绝不能绕过它
- 某个模块虽然都改同一目录，但语义边界其实在哪里
...
- 每个已读文件为什么相关
- 从这些文件中得出的关键结论
- 已读文件中哪些部分对后续继续工作真正有用
- 这些有用内容里，哪些已经足够确认，可以不必重读原文件
...
- `current-work.md`：默认不超过 420 行；如果为了保留高价值探索索引确实需要更长，可以适度放宽，但仍必须主动去重、删过时内容，不能写成流水账
...
- `project-understanding.md`
  放项目唯一事实来源，包括长期工作规则、架构边界、重要设计理解、模块地图、关键运行时语义、默认验证策略、禁止路径
- `current-work.md`
  只放当前任务现场
...
- 优先记录“文件里真正有用的部分”，而不是笼统记录“整个文件读过”
- 如果某部分内容已经被提炼得足够支撑下一轮继续工作，应明确标记可不必重读原文件

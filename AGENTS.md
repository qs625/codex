...
- root-worker composer slash 菜单中，能表达为 Skill 的命令应来自 Skills discovery；例如 `/init` 由 embedded system skill 提供，不要作为 root-worker builtin command 硬编码。只有依赖 runtime/thread state 或客户端本地动作的命令（例如 `/clear`、`/goal cancel`）才放入 root-worker builtin slash command registry，并且执行时不得作为普通 user message 发送给模型。

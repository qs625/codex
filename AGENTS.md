...
- live `item/started` 和 `item/completed` payload 的展示语义以 typed `TurnItem` 为 canonical source；app-server v2 的 `ThreadItem` 应复用 `codex-rs/app-server-protocol/src/protocol/v2/item.rs` 中的 `TurnItem -> ThreadItem` 转换，不要为 live lifecycle 新增 raw response item 或 message marker 解析路径。

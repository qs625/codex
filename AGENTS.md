...
- Dynamic Workflow 当前只实现 registry/init context 和 `workflow_list`、`workflow_describe` 工具：home workflow 位于 `$CODEX_HOME/workflows/<workflow-id>/`，project workflow 位于各 active project `.codex/workflows/<workflow-id>/`，project 同 id 覆盖 home；`workflow.json.id` 必须等于目录名，`entry` 必须指向同目录内存在的 TypeScript 文件。runner、`workflow_start/status/resume/abort`、app-server v2 控制面和 typed workflow progress item 仍属于后续实现，新增时必须继续走 typed `ResponseItem -> ThreadItem` 路径。

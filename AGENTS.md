...
- 对话/线程展示相关的结构化语义以 typed `ResponseItem` 为 canonical source；`ThreadItem` 应通过共享 projector 统一生成。新增或修改 event-command、schedule、collab 这类展示项时，不要再引入从 message marker 文本解析 typed item 的新逻辑；优先复用 `codex-rs/app-server-protocol/src/protocol/response_item_projection.rs`。

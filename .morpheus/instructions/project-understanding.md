...
- 安装态 Root Worker/Morpheus desktop 的签名 App Bundle 是稳定只读壳：外层只包含通用 Runtime Capsule Launcher 和完整签名的 Seed Capsule。可更新的 Main/preload/renderer/app-server/default config 必须作为同一 release 的完整 verified Runtime Capsule 存放在 `MORPHEUS_HOME/runtime-launcher/artifacts/<release-id>`；Launcher 只保留 external current/previous 两代，Seed 独立于轮转并作为最终 fallback。普通 external 激活、回滚和 commit 不得写 App Bundle 或在 activation-time 重新 codesign。
- 安装态 source workspace 是可选的候选生产能力，不是启动依赖。packaged app 不自动 clone、build、pull 或 reset 源码；没有有效 source workspace 时，Seed 或已有 external runtime 仍应正常运行，`request_runtime_restart` 返回 typed disabled/unsupported。只有检测到真实源码结构时才维护 `MORPHEUS_HOME/instructions/morpheus-source-workspace.md`。
- `/self` 是指向 Morpheus 自身真实源码 workspace 的 system self project contract；只有 workspace 存在且包含预期源码结构时，packaged Root Worker 才能 materialize `/self` root thread。不得为缺失、空目录或普通 operational workspace 创建 `/self`。self command IPC 应只向 `/self` 发送任务，并确保 thread cwd/workspace 是实际源码 workspace。Cmd+P 类命令板的产品方向是专用 `/self` 输入面，不是通用 project selector。
- Runtime Capsule 更新只有一种 model-visible 完整 Runtime 重启：候选激活前必须按受信 Capsule 的 cooperative supervision contract 停止旧 Runtime 的已观察进程树，再切换 selection 并启动新 Capsule；Launcher 不提供 Hot/Full mode 或 in-place payload swap。Capsule manifest 必须明确禁止 daemonize、double-fork、`setsid` 和 process-group escape；观察到逃逸、身份不符、超时、归属歧义或残留时必须在 selection、fallback、commit 或新 spawn 前 durable Blocked。macOS/Linux v1 的证据只能称为 typed `CooperativeObservedEmpty`，不能宣称 kernel-contained 或敌对同 UID payload 的完整树强证明；Linux cgroup v2 strong backend 是后续非目标。legacy `hot` 请求必须 typed reject，legacy `full` 只可兼容映射到同一完整 Capsule 重启语义。候选 ready/观察失败时回滚到 external previous；没有 previous 时回到只读 Seed。
...
- successful compact 的 fresh instruction/agent-role/external-tool context 必须在 PostCompact hook Continue 后一次性物化。`BeforeLastUserMessage` 注入会让 replacement history 与 typed reference/display context共享该快照，并提交其中的 user-instructions session baseline；pre-turn `DoNotInject` 不注入 response/reference context，只原子更新 fresh user-instructions session baseline并保持 reference 为 None，下一 regular turn再从已提交 baseline与当时的 fresh role/tool context完成完整注入。`Compacted + optional TurnContext` 必须作为完整 durable checkpoint 写入并 flush 后，才允许发布新的 segment head 或更新 live/session baseline；失败或中断不能暴露部分 compact head。

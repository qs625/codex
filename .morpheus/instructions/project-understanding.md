...
- 安装态会维护一份受控 instruction 文件 `MORPHEUS_HOME/instructions/morpheus-source-workspace.md`，告诉模型当前 Morpheus source workspace 路径，以及修改 runtime/client/server/frontend/backend 后应先完成相关测试，再调用 `request_runtime_restart`。packaged macOS app 的真实 OS 入口是稳定 Rust `MorpheusLauncher`，Electron host 只使用 source workspace 中已存在的构建产物准备带 manifest/hash 的候选 runtime；Launcher 本身不运行 cargo/pnpm，也不允许候选替换 Launcher 代码或路径。
- macOS installed runtime 更新由 `MorpheusLauncher` 持有 durable current/previous/transaction 状态：full 模式通过协调退出后切换并等待 app-server 与 renderer ready identity，hot 模式由 host 显式激活、重启 app-server、reload renderer 后提交；切换、签名、验证、ready 前失败及 post-ready crash-loop 都必须可恢复或回滚。未确认的 failure evidence 会跨 Launcher 重启继续注入，并通过 trusted `/self` recovery RPC 进入 typed `EventMsg -> ThreadItem`、Limited rollout 与 reload replay；不能退回普通隐藏文本或 raw marker。

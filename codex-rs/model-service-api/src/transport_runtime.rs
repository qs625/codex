...
use crate::ApiError;
use crate::ApiRuntimeFuture;
use crate::ArcMonitorRuntimeRequest;
use crate::ArcMonitorRuntimeResponse;
use crate::ChatCompletionsRuntimeRequest;
use crate::CompactInputRuntimeRequest;
use crate::CompactionOutput;
use crate::MemorySummarizeOutput;
use crate::MemorySummarizeRuntimeRequest;
use crate::RealtimeCallResponse;
use crate::RealtimeCallRuntimeRequest;
use crate::RealtimeWebsocketClientRuntime;
use crate::Provider;
use crate::ResponseStream;
use crate::ResponsesStreamRuntimeRequest;
use crate::ResponsesWebsocketConnectorRuntime;
...
/// depending on a concrete model API implementation crate.
...
impl ResponsesWebsocketConnectorRuntime for DisabledResponsesWebsocketConnectorRuntime {
    fn connect(
        &self,
        _request: crate::ResponsesWebsocketConnectRequest,
    ) -> crate::ApiRuntimeFuture<'_, Result<Box<dyn crate::ResponsesWebsocketConnectionRuntime>, ApiError>>
    {
        Box::pin(async {
            Err(ApiError::Stream(
...
impl RealtimeWebsocketClientRuntime for DisabledRealtimeWebsocketClientRuntime {
    fn connect(
        &self,
        _request: crate::RealtimeWebsocketConnectRuntimeRequest,
    ) -> ApiRuntimeFuture<'_, Result<Box<dyn crate::RealtimeWebsocketConnectionRuntime>, ApiError>>
    {
        Box::pin(async {
            Err(ApiError::Stream(
    ...
    fn connect_webrtc_sideband(
        &self,
        _request: crate::RealtimeWebrtcSidebandConnectRuntimeRequest,
    ) -> ApiRuntimeFuture<'_, Result<Box<dyn crate::RealtimeWebsocketConnectionRuntime>, ApiError>>
    {
        Box::pin(async {
            Err(ApiError::Stream(

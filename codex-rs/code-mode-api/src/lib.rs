...
pub type CodeModeBoxFuture<'a, T> = Pin<Box<dyn Future<Output = T> + Send + 'a>>;
pub type CodeModeBoxResultFuture<'a, T> =
    Pin<Box<dyn Future<Output = Result<T, String>> + Send + 'a>>;

/// Host callbacks that a code-mode runtime uses to deliver nested tool calls
/// and runtime notifications back into the owning turn.
...
/// Opaque per-turn worker handle returned by a code-mode runtime.
///
/// Implementations should stop listening for runtime turn messages when this
/// handle is dropped.
pub trait CodeModeTurnWorker: Send {}

/// Session-scoped code-mode runtime boundary used by core/session orchestration.
///
/// Implementations own execution state such as live cells and stored values.
/// Each model session must receive an independent service instance.
pub trait CodeModeRuntimeService: Send + Sync {
    ...
    fn start_turn_worker(&self, host: Arc<dyn CodeModeTurnHost>) -> Box<dyn CodeModeTurnWorker>;
}

/// Factory for creating a fresh session-scoped code-mode runtime service.
pub trait CodeModeRuntimeFactory: Send + Sync {
    fn create_service(&self) -> Arc<dyn CodeModeRuntimeService>;
}

/// Runtime factory for contexts that need code-mode types but do not execute
/// code-mode cells.
#[derive(Default)]
pub struct DisabledCodeModeRuntimeFactory;

impl CodeModeRuntimeFactory for DisabledCodeModeRuntimeFactory {
    fn create_service(&self) -> Arc<dyn CodeModeRuntimeService> {
        Arc::new(DisabledCodeModeRuntimeService)
    }
}

/// Runtime service that rejects execution and keeps no state.
///
/// This is intended for test/support paths that construct sessions only to
/// inspect prompts or thread metadata.
pub struct DisabledCodeModeRuntimeService;

impl CodeModeRuntimeService for DisabledCodeModeRuntimeService {
    fn stored_values(&self) -> CodeModeBoxFuture<'_, HashMap<String, JsonValue>> {
    ...
    fn replace_stored_values(
        &self,
        _values: HashMap<String, JsonValue>,
    ) -> CodeModeBoxFuture<'_, ()> {
        Box::pin(async {})
    }
    ...
    fn execute(&self, _request: ExecuteRequest) -> CodeModeBoxResultFuture<'_, RuntimeResponse> {
        Box::pin(async { Err("code-mode runtime is not available".to_string()) })
    }
    ...
    fn wait(&self, request: WaitRequest) -> CodeModeBoxResultFuture<'_, WaitOutcome> {
        Box::pin(async move {
            Ok(WaitOutcome::MissingCell(RuntimeResponse::Result {
                ...
                error_text: Some("code-mode runtime is not available".to_string()),
            }))
        })
    }

    fn start_turn_worker(&self, _host: Arc<dyn CodeModeTurnHost>) -> Box<dyn CodeModeTurnWorker> {
        Box::new(DisabledCodeModeTurnWorker)
    }
}

struct DisabledCodeModeTurnWorker;

impl CodeModeTurnWorker for DisabledCodeModeTurnWorker {}

...
/// Command policy rule implementations match command tokens and expose their program key.
///
/// Implementations are expected to be immutable, thread-safe, and downcastable through `as_any`
/// when callers need to inspect concrete rule data for rendering or fingerprinting.
...
pub fn validate_match_examples(
    policy: &Policy,
    rules: &[RuleRef],
    matches: &[Vec<String>],
) -> Result<()> {

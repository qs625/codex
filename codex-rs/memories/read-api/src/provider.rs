...
/// Provides memory-tool developer instructions to a session runtime.
///
/// Implementations own any filesystem, template, or remote lookup needed to
/// build the prompt. `codex-core` consumes this trait so it does not depend on
/// the concrete memory read-path implementation.

...
- "Where should I create this skill? If you do not have a preference, I will place it in `$MORPHEUS_HOME/skills` (or `~/.morpheus/skills` when `MORPHEUS_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$MORPHEUS_HOME/skills`; when `MORPHEUS_HOME` is unset, fall back to `~/.morpheus/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${MORPHEUS_HOME:-$HOME/.morpheus}/skills"
scripts/init_skill.py my-skill --path "${MORPHEUS_HOME:-$HOME/.morpheus}/skills" --resources scripts,references

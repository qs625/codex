...
use protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.morpheus".to_string(),
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // write to .morpheus/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.morpheus carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    // Verify that .morpheus/config.toml cannot be modified under the generated
    // Seatbelt policy.
    ...
    // Verify that writing a file to the folder containing .git and .morpheus is allowed.
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .morpheus/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".morpheus")).expect("create .morpheus");
    fs::write(
        vulnerable_root.join(".morpheus").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .morpheus/config.toml");
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".morpheus");

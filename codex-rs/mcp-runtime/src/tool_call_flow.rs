...
// without making codex-mcp-runtime depend on the pty crate for a constant.
...
pub trait McpToolCallHost: McpApprovedToolCallLifecycleHost {
    fn lookup_mcp_tool_metadata(
        &self,
        server: &str,
        tool_name: &str,
    ) -> impl std::future::Future<Output = Option<McpToolApprovalMetadata>> + Send;
...
pub async fn handle_mcp_tool_call(
    ...
    let metadata = host
        .lookup_mcp_tool_metadata(&context.server, &context.tool_name)
        .await;

...
impl TurnDiffTracker {
    ...
    fn render_diff(
        ...
        let left_header = if left_content.is_some() {
            format!("a/{left_display}")
        } else {
            DEV_NULL.to_string()
        };
        let right_header = if right_content.is_some() {
            format!("b/{right_display}")
        } else {
            DEV_NULL.to_string()
        };
        diff.push_str(&format!("index {left_oid}..{right_oid} {REGULAR_FILE_MODE}\n"));
        diff.push_str(&format!("--- {left_header}\n"));
        diff.push_str(&format!("+++ {right_header}\n"));
        diff.push_str(&unified_diff_from_texts(

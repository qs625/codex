...
pub enum ConfigEdit {
    ...
    SetModelPersonality { personality: Option<String> },
...
impl ConfigEdit {
    pub fn set_string_path(segments: Vec<String>, value: impl Into<String>) -> Self {
        Self::SetPath {
            segments,
            value: toml_edit::value(value.into()),
        }
    }
}
        ...
        match edit {
            ConfigEdit::SetModel { model, effort } => Ok({
                ...
                mutated |= self.write_profile_value(
                    ...
                    effort.as_ref().map(|effort| value(effort.clone())),
                );
                mutated
            }),
            ConfigEdit::SetServiceTier { service_tier } => Ok(self.write_profile_value(
                ...
                service_tier.as_ref().map(|service_tier| {
                    let config_value =
                        match ServiceTierConfigValue::from_request_value(service_tier) {
                            Some(ServiceTierConfigValue::Fast) => "fast",
                            Some(ServiceTierConfigValue::Flex) => "flex",
            ...
            ConfigEdit::SetModelPersonality { personality } => Ok(self.write_profile_value(
                ...
                personality
                    .as_ref()
                    .map(|personality| value(personality.clone())),
            )),
            ...
            ConfigEdit::SetProjectTrustLevel { path, level } => {
                set_project_trust_level_in_document(&mut self.doc, path.as_path(), level)?;
                Ok(true)
            }
        }
    }
...
/// Set trust_level under `[projects."<path>"]`, migrating inline tables to explicit tables.
pub fn set_project_trust_level_in_document(
    doc: &mut DocumentMut,
    project_path: &Path,
    trust_level: impl ToString,
) -> anyhow::Result<()> {
    ...
    // replace it with an explicit table.
    {
        ...
        if existing_projects.as_ref().is_none_or(|i| !i.is_table()) {
            let mut projects_tbl = TomlTable::new();
            ...
            if let Some(inline_tbl) = existing_projects.as_ref().and_then(|i| i.as_inline_table()) {
                for (key, value) in inline_tbl.iter() {
                    if let Some(inner_tbl) = value.as_inline_table() {
                        projects_tbl.insert(key, TomlItem::Table(inner_tbl.clone().into_table()));
                    }
                }
            }

            root.insert("projects", TomlItem::Table(projects_tbl));
        }
    }
    ...
    proj_tbl["trust_level"] = value(trust_level.to_string());
    Ok(())
}

enum ServiceTierConfigValue {
    Fast,
    Flex,
}

impl ServiceTierConfigValue {
...
pub struct ConfigEditsBuilder {
    config_path: PathBuf,
    profile: Option<String>,
    default_false_feature_keys: HashSet<String>,
    edits: Vec<ConfigEdit>,
}
...
impl ConfigEditsBuilder {
    ...
    pub fn for_config_path(config_path: &Path) -> Self {
        Self {
            ...
            profile: None,
            default_false_feature_keys: HashSet::new(),
            edits: Vec::new(),
        }
    }
    ...
    pub fn with_default_false_feature_keys<I, S>(mut self, keys: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        self.default_false_feature_keys = keys.into_iter().map(Into::into).collect();
        self
    }

    pub fn set_model(mut self, model: Option<&str>, effort: Option<String>) -> Self {
    ...
    pub fn set_personality(mut self, personality: Option<String>) -> Self {
        self.edits
    ...
    pub fn set_feature_enabled(mut self, key: &str, enabled: bool) -> Self {
        ...
        let is_default_false_feature = self.default_false_feature_keys.contains(key);

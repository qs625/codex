...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to MORPHEUS_HOME or anything under MORPHEUS_HOME/.sandbox,
    // MORPHEUS_HOME/.sandbox-bin, or MORPHEUS_HOME/.sandbox-secrets. These locations contain sandbox
...
mod tests {
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_morpheus = extra_write_root.join(".morpheus");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_morpheus).expect("create extra .morpheus");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_morpheus).expect("canonical extra .morpheus"),
                explicit_deny,
            ]
            .into_iter()

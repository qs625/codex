...
* Use `wait_agent` only for sub-agent mailbox updates, not for waiting on local commands or file changes. Use `process_exit_subscribe` for local process completion and `fs_subscribe` for file or log watching.

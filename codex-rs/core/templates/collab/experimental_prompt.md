...
* Use `wait_agent` only for sub-agent mailbox updates, not for waiting on local commands or file changes. Use `event_command_subscribe` for local command completion or file/log watching by running a quiet command that emits one event per stdout line.

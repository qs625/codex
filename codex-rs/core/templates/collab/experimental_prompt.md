...
* Sub-agent completion and mailbox updates are delivered automatically to the parent. Do not poll for them.
* For local command completion or file/log watching, use `event_command_subscribe` by running a quiet command that emits one event per stdout line.

...
use super::resolve_use_login_shell;
use super::shell_command_exec_params;
...
/// The logic for is_known_safe_command() has heuristics for known shells, so
/// we must ensure core shell command construction can be recognized as safe if
/// the model command is safe.
#[test]
...
async fn shell_command_handler_to_exec_params_uses_session_shell_and_turn_context() {
    ...
    let exec_params = shell_command_exec_params(
        &params,
        &session,
        &turn_context,
    ...
    assert_eq!(
        exec_params.network.is_some(),
        turn_context.network.is_some()
    );
    if let (Some(actual), Some(expected)) = (&exec_params.network, &turn_context.network) {
...
fn shell_command_handler_respects_explicit_login_flag() {
    ...
    let login_command = shell.derive_exec_args("echo login shell", /*use_login_shell*/ true);
    assert_eq!(
        login_command,
...
fn shell_command_handler_rejects_login_when_disallowed() {
    let err = resolve_use_login_shell(Some(true), /*allow_login_shell*/ false)

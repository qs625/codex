...
fn write_permissions_for_paths_skip_dirs_already_writable_under_workspace_root() {
    ...
    let file_path =
        AbsolutePathBuf::try_from(nested.join("file.txt")).expect("absolute nested file path");
...
fn write_permissions_for_paths_keep_dirs_outside_workspace_root() {
    ...
    let file_path =
        AbsolutePathBuf::try_from(outside.join("file.txt")).expect("absolute outside file path");
    let cwd_abs = abs(&cwd);
    let sandbox_policy = FileSystemSandboxPolicy::from(&SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        network_access: false,
    ...
    let expected_outside = abs(&outside.canonicalize().expect("canonicalize outside dir"));

...
fn shell_with_snapshot(
    ...
    RuntimeShell {
        ...
        shell_snapshot: Some(RuntimeShellSnapshot {
            path: snapshot_path,
            cwd: snapshot_cwd,
        }),
    }
...
#[test]
fn explicit_escalation_removes_codex_managed_proxy_env() {
    let mut env = HashMap::from([
        (PROXY_ACTIVE_ENV_KEY.to_string(), "1".to_string()),
        ("CUSTOM_ENV".to_string(), "kept".to_string()),
    ]);
    for key in PROXY_ENV_KEYS {
        env.insert((*key).to_string(), format!("managed-{key}"));
    }
    #[cfg(target_os = "macos")]
    env.insert(
        PROXY_GIT_SSH_COMMAND_ENV_KEY.to_string(),
        format!("{CODEX_PROXY_GIT_SSH_COMMAND_MARKER} /tmp/socket"),
    );

    let env = exec_env_for_sandbox_permissions(&env, SandboxPermissions::RequireEscalated);

    for key in PROXY_ENV_KEYS {
        assert_eq!(env.get(*key), None, "{key} should be unset");
    }
    #[cfg(target_os = "macos")]
    assert_eq!(env.get(PROXY_GIT_SSH_COMMAND_ENV_KEY), None);
    assert_eq!(env.get("CUSTOM_ENV"), Some(&"kept".to_string()));
}
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_preserves_thread_id_after_snapshot() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(&snapshot_path, "# Snapshot file\n").expect("write snapshot");
    let session_shell = shell_with_snapshot(
    ...
    let env = HashMap::from([(CODEX_THREAD_ID_ENV_VAR.to_string(), "thread-1".to_string())]);
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "echo hello".to_string(),
    ...
    assert!(rewritten[2].contains(CODEX_THREAD_ID_ENV_VAR));
    assert!(rewritten[2].contains("__CODEX_SNAPSHOT_OVERRIDE"));
}

#[test]
fn maybe_wrap_shell_lc_with_snapshot_leaves_non_login_shell_alone() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(&snapshot_path, "# Snapshot file\n").expect("write snapshot");
    let session_shell = shell_with_snapshot(
...
fn inserts_no_profile_for_elevated_windows_sandbox() {
    ...
    let rewritten = disable_powershell_profile_for_elevated_windows_sandbox(
        &command,
        Some(ToolUserShellType::PowerShell),
...
#[test]
fn leaves_existing_no_profile_alone() {
    let command = vec![
        "pwsh.exe".to_string(),
        "-NoProfile".to_string(),
        "-Command".to_string(),

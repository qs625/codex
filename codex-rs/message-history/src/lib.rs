...
//!
//! The history is stored at `~/.morpheus/history.jsonl` with **one JSON object per
...
use codex_config_types::HistoryPersistence;
...
/// Filename that stores the message history inside `~/.morpheus`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.morpheus/history.jsonl` and ensure the parent directory exists.

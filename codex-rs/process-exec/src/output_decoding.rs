...
//! Unicode replacement character. Runtime command output decoding uses `chardetng` and
//! `encoding_rs` so we can automatically detect and decode the vast majority of legacy encodings
//! before falling back to lossy UTF-8 decoding.

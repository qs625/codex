            ...
            for (group_index, group) in groups.iter().enumerate() {
                for (handler_index, _) in group.hooks.iter().enumerate() {
                    declarations.push(PluginHookDeclaration {
                        key: hook_key(&key_source, event_name, group_index, handler_index),
                        event_name,
                    });
                }
            }
...
/// Builds the stable persisted key prefix for hook declarations from one plugin hook source.
pub fn plugin_hook_key_source(plugin_id: &str, source_relative_path: &str) -> String {

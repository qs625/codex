...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let _skill_service = SkillService::new(
        codex_home.path().abs(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let outcome1 =
        skills_for_config_with_stack(&skill_service, &cwd, &config_layer_stack, &[]).await;
    assert!(
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_codex = cwd.path().join(".morpheus");
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    let _ = skills_for_config_with_stack(&skill_service, &cwd, &config_layer_stack, &[]).await;
    ...
    let outcome_reloaded = skill_service
        .skills_for_cwd(
            &base_input,
            /*force_reload*/ true,
            Some(Arc::clone(&LOCAL_FS)),
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_outcome = skill_service
        .skills_for_cwd(
            &parent_input,
            /*force_reload*/ true,
            Some(Arc::clone(&LOCAL_FS)),
    ...
    let child_outcome = skills_for_config_with_stack(&skill_service, &cwd, &child_stack, &[]).await;

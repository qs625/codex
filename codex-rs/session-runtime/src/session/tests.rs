...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let (mailbox, mailbox_rx) = codex_session_runtime::Mailbox::new();
    let session = Session {
...
async fn shutdown_and_wait_shuts_down_cached_guardian_subagent() {
    ...
    let child_reuse_key = crate::guardian::GuardianReviewSessionReuseKey::from_spawn_config(
...
async fn inter_agent_child_completion_live_item_waits_for_typed_recording() -> anyhow::Result<()> {
    ...
    codex_hooks::record_pending_input(
        session.as_ref(),
        turn_context.as_ref(),
        codex_hooks::PendingInputRecord::InterAgentCommunication {
...
async fn create_goal_tool_rejects_existing_goal() {
    ...
    let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::new()));
    let handler =
        codex_tool_handlers::CreateGoalHandler::new(codex_session_api::SessionGoalHost::<
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let create_handler =
        ...
        >::default());
    let update_handler =
        codex_tool_handlers::UpdateGoalHandler::new(codex_session_api::SessionGoalHost::<
            SharedTurnDiffTracker,
            TurnContext,
        >::default());

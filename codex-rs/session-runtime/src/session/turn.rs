...
use crate::tools::extension_tools::extension_tool_executors;
...
use codex_tool_runtime::ToolCallRuntimeRouter;
...
pub(crate) struct InjectedToolRuntimeRouter {
    router: Arc<crate::CoreToolRuntimeRouter>,
}

impl InjectedToolRuntimeRouter {
    pub(crate) fn new(router: Arc<crate::CoreToolRuntimeRouter>) -> Self {
        Self { router }
    }
}

impl ToolCallRuntimeRouter<Arc<Session>, Arc<TurnContext>, SharedTurnDiffTracker, TurnContext>
    for InjectedToolRuntimeRouter
{
    ...
    ) -> impl Future<Output = Result<codex_tool_runtime::AnyToolResult, FunctionCallError>> + Send
    {
        self.router.dispatch_tool_call_with_code_mode_result(
            session,
            turn,
            cancellation_token,
            tracker,
...
async fn run_sampling_request(
    ...
    let tool_runtime = ToolCallRuntime::new(
        Arc::new(InjectedToolRuntimeRouter::new(Arc::clone(&router))),
        Arc::clone(&sess),
        Arc::clone(&turn_context),
        Arc::clone(&turn_diff_tracker),
    );
...
pub(crate) async fn built_tools(
    ...
    skills_outcome: Option<&SkillLoadOutcome>,
    cancellation_token: &CancellationToken,
) -> CodexResult<Arc<crate::CoreToolRuntimeRouter>> {
    ...
    let router = sess.services.tool_router_factory.build_tool_router(
        &turn_context.tools_config,

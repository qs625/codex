...
async fn mcp_elicitation_response_from_guardian_decision(
    ...
    let denial_message = match decision {
        ReviewDecision::Denied => {
            Some(approval_service::guardian::guardian_rejection_message(session, review_id).await)
        }
        _ => None,
    };
...
fn mcp_elicitation_response_from_guardian_decision_parts(
    ...
    match decision {
        ...
        ReviewDecision::TimedOut => {
            mcp_elicitation_decline_with_message(approval_service::guardian::guardian_timeout_message())
        }

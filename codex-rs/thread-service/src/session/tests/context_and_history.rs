...
#[tokio::test]
async fn client_recovery_record_is_durable_before_turn_owned_model_input() {
    let (mut sess, tc) = make_session_and_context().await;
    let rollout_path = attach_thread_persistence(&mut sess).await;
    let recovery_identity = "11111111-1111-4111-8111-111111111111";
    ...
    let expected_event = ClientRecoveryRecordedEvent {
        id: recovery_id.clone(),
        turn_id: recovery_id.clone(),
        recovery_identity: Some(recovery_identity.into()),
        ...
        requested_by_thread_id: Some("requester".into()),
        mode: "full".into(),
    ...
    sess.record_client_recovery_event(
        recovery_id.clone(),
        EventMsg::ClientRecoveryRecorded(expected_event.clone()),
    )
    .await
    .expect("persist recovery event");
    assert!(
        sess.clone_history().await.raw_items().is_empty(),
        "recording durable evidence must not mutate an active turn's model history"
    );
    let disposition = hooks::inspect_pending_input(
        &sess,
        &tc,
        PendingInputItem::RecordedResponseItem {
            response_item: input.clone(),
            recovery_id: recovery_id.clone(),
        },
    )
    .await;
    let hooks::PendingInputHookDisposition::Accepted(record) = disposition else {
        panic!("recorded recovery input should be accepted");
    };
    assert_eq!(record.recovery_id(), Some(recovery_id.as_str()));
    let hooks::PendingInputRecord::RecordedConversationItem {
        recovery_id: accepted_recovery_id,
        response_item,
    } = *record
    else {
        panic!("expected recorded recovery input");
    };
    sess.record_client_recovery_input_for_turn(
        &tc,
        accepted_recovery_id.clone(),
        &response_item,
    )
    .await;
    ...
    sess.record_client_recovery_input_for_turn(&tc, accepted_recovery_id, &response_item)
        .await;
    assert_eq!(
        sess.clone_history().await.raw_items(),
        std::slice::from_ref(&input),
        "duplicate recovery scheduling must not duplicate model-visible history"
    );
    sess.flush_rollout().await.expect("flush recovery rollout");
    let InitialHistory::Resumed(resumed) = RolloutRecorder::get_rollout_history(&rollout_path)
        .await
        .expect("read recovery rollout")
    else {
        panic!("expected resumed recovery rollout");
    };
    let recorded_event_positions = resumed
        .history
        .iter()
        .enumerate()
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        recorded_event_positions.len(),
        1,
        "the complete typed recovery fact must be persisted once without payload drift"
    );
    let recorded_event_position = recorded_event_positions[0];
    let durable_input = ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
            text: "Inspect recovery evidence.".to_string(),
    ...
    let durable_input_positions = resumed
        .history
        .iter()
        .enumerate()
        .filter_map(|(position, item)| {
            matches!(
                item,
                RolloutItem::ResponseItem(response_item) if response_item == &durable_input
            )
            .then_some(position)
        })
        .collect::<Vec<_>>();
    assert_eq!(
        durable_input_positions.len(),
        1,
        "the turn that owns the recovery must persist its model input once"
    );
    let durable_input_position = durable_input_positions[0];
    assert!(
        recorded_event_position < durable_input_position,
        "the typed recovery fact must be durable before its turn-owned model input"
    );
}

#[tokio::test]
async fn client_recovery_batch_fails_before_history_without_durable_thread_store() {
    let (sess, _tc) = make_session_and_context().await;
    let event = EventMsg::ClientRecoveryRecorded(ClientRecoveryRecordedEvent {
        id: "client-recovery:tx-1:req-1".into(),
        turn_id: "client-recovery-tx-1".into(),
    ...
    let result = sess
        .record_client_recovery_event("client-recovery-tx-1".into(), event)
        .await;
...
#[tokio::test]
async fn client_recovery_retry_keeps_one_marker_and_wakes_idle_duplicate() {
    let (sess, _tc) = make_session_and_context().await;
    let input = ResponseItem::Message {
    ...
    assert!(
        sess.enqueue_recorded_recovery_for_next_turn_if_absent(
            input.clone(),
            "client-recovery:tx-1:req-1".into(),
        )
        .await
    );
    assert!(
        ...
        .await,
        "an idle duplicate must wake the existing marker without adding another"
    );
    let queued = sess.take_queued_response_items_for_next_turn().await;
    assert_eq!(queued.len(), 1);
    assert_eq!(
        queued[0].recovery_id(),
        Some("client-recovery:tx-1:req-1")
    );
}

#[tokio::test]
async fn queued_client_recovery_remains_visible_across_terminal_handoff() {
    let (sess, _tc) = make_session_and_context().await;
    let active_turn = ActiveTurn::default();
    active_turn.turn_state.lock().await.mark_terminal_handoff();
    *sess.active_turn.lock().await = Some(active_turn);
    ...
    assert!(
        !sess
            .enqueue_recorded_recovery_for_next_turn_if_absent(
                input,
                "client-recovery:tx-1:req-1".into(),
            )
            .await,
        "an active terminal turn must defer the recovery wake"
    );
    *sess.active_turn.lock().await = None;
    assert!(
        sess.has_queued_client_recovery_excluding(&std::collections::HashSet::new())
            .await,
        "turn completion must prioritize the deferred recovery over terminal handoff"
    );
}

#[tokio::test]
async fn active_client_recovery_duplicate_does_not_start_another_turn() {
    let (sess, _tc) = make_session_and_context().await;
    let input = ResponseItem::Message {
        id: Some("client-recovery:tx-1:req-1".to_string()),
        role: "user".to_string(),
    ...
    *sess.active_turn.lock().await = Some(active_turn);
    ...
    assert!(
        !sess
            ...
            .await,
        "an active turn that owns the recovery must suppress duplicate starts"
    );
    assert!(
        !sess
            .enqueue_recorded_recovery_for_next_turn_if_absent(
            ...
            .await,
        "multiple active duplicates must still suppress immediate starts"
    );
    let active = sess.active_turn.lock().await;
    let turn_state = active.as_ref().expect("active turn").turn_state.lock().await;
    assert!(turn_state.has_client_recovery_retry_wake(
        "client-recovery:tx-1:req-1"
    ));
    assert_eq!(
        turn_state.client_recovery_retry_wakes().len(),
        1,
        "multiple duplicate RPCs must coalesce into one post-turn wake"
    );
}

#[tokio::test]
async fn failed_recovery_marker_is_not_misclassified_as_new_external_wake() {
    let (sess, _tc) = make_session_and_context().await;
    let recovery_input = |id: &str| ResponseItem::Message {
        id: Some(id.to_string()),
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
            text: format!("Inspect {id}."),
        }],
        phase: None,
    };
    sess.enqueue_recorded_recovery_for_next_turn_if_absent(
        recovery_input("client-recovery:tx-1:req-1"),
        "client-recovery:tx-1:req-1".into(),
    )
    .await;
    let cold_recovery_ids =
        std::collections::HashSet::from(["client-recovery:tx-1:req-1".to_string()]);

    assert!(
        !sess
            .has_queued_client_recovery_excluding(&cold_recovery_ids)
            .await,
        "a pre-drain failure must remain pending without triggering a hot loop"
    );
    assert!(
        !sess
            .has_thread_pending_work_excluding_recoveries(&cold_recovery_ids)
            .await,
        "cold recovery requeue must not be treated as ordinary pending work"
    );

    sess.enqueue_recorded_recovery_for_next_turn_if_absent(
        recovery_input("client-recovery:tx-2:req-2"),
        "client-recovery:tx-2:req-2".into(),
    )
    .await;
    assert!(
        sess.has_queued_client_recovery_excluding(&cold_recovery_ids)
            .await,
        "a genuinely new recovery ID must still wake the idle thread"
    );
}

#[tokio::test]
async fn draining_client_recovery_registers_payload_before_marker_leaves_turn_state() {
    let (sess, _tc) = make_session_and_context().await;
    let input = ResponseItem::Message {
        id: Some("client-recovery:tx-1:req-1".to_string()),
        role: "user".to_string(),
    ...
    active_turn
        ...
        .lock()
        .await
        .push_pending_input(PendingInputItem::RecordedResponseItem {
            response_item: input.clone(),
            recovery_id: "client-recovery:tx-1:req-1".into(),
        });
    *sess.active_turn.lock().await = Some(active_turn);
    ...
    let turn_state = active.as_ref().expect("active turn").turn_state.lock().await;
    assert_eq!(
        turn_state.client_recoveries(),
        vec![(input, "client-recovery:tx-1:req-1".into())],
        "abort must be able to recover the payload after the marker has been drained"
    );
}

#[test]
fn client_recovery_success_is_frozen_per_id_across_multiple_runs() {
    let mut turn_state = crate::state::TurnState::default();
    let first = ResponseItem::Message {
        id: Some("client-recovery:tx-1:req-1".to_string()),
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
            text: "Inspect first recovery.".to_string(),
        }],
        phase: None,
    };
    let second = ResponseItem::Message {
        id: Some("client-recovery:tx-2:req-2".to_string()),
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
            text: "Inspect second recovery.".to_string(),
        }],
        phase: None,
    };

    turn_state.record_client_recovery(
        "client-recovery:tx-1:req-1".into(),
        first.clone(),
    );
    turn_state.begin_client_recovery_run();
    turn_state.mark_client_recovery_run_succeeded();
    turn_state.begin_client_recovery_run();
    turn_state.record_client_recovery(
        "client-recovery:tx-2:req-2".into(),
        second.clone(),
    ...
    let (succeeded, pending) = turn_state.client_recoveries_by_completion();
    assert_eq!(
        succeeded,
        vec![(first, "client-recovery:tx-1:req-1".into())]
    );
    assert_eq!(
        pending,
        vec![(second, "client-recovery:tx-2:req-2".into())],
        "a later failed run must not erase an earlier recovery success"
    );
}

#[test]
fn client_recovery_remains_pending_without_explicit_success_mark() {
    let mut turn_state = crate::state::TurnState::default();
    let input = ResponseItem::Message {
        id: Some("client-recovery:tx-1:req-1".to_string()),
        role: "user".to_string(),
    ...
    assert_eq!(
        pending,
        vec![(input, "client-recovery:tx-1:req-1".into())],
        "stop veto, after-agent abort, provider failure, and context failure must all remain pending"
    );
}

#[tokio::test]
async fn handled_persistence_failure_can_requeue_full_recovery_payload() {
    ...
    assert!(
        sess.record_client_recoveries_handled(
            &tc,
            vec!["client-recovery:tx-1:req-1".into()],
        )
        .await
        .is_err(),
        "Handled must fail when no durable live thread is available"
    );
    assert!(
        sess.enqueue_client_recoveries_for_next_turn(vec![(
            input.clone(),
            "client-recovery:tx-1:req-1".into(),
        )])
        .await
    ...
    assert_eq!(
        ...
        vec![PendingInputItem::RecordedResponseItem {
            response_item: input,
            recovery_id: "client-recovery:tx-1:req-1".into(),
        }]
    );
...
#[tokio::test]
async fn handled_client_recovery_is_durably_persisted() {
    let (mut sess, tc) = make_session_and_context().await;
    let rollout_path = attach_thread_persistence(&mut sess).await;
    let recovery_identity = "11111111-1111-4111-8111-111111111111";
    let recovery_id = format!("client-recovery:{recovery_identity}");
    ...
    .await
    .expect("persist handled recovery");
    let InitialHistory::Resumed(resumed) = RolloutRecorder::get_rollout_history(&rollout_path)
        .await
        .expect("read recovery rollout")
    else {
        ...
        matches!(
            ...
            RolloutItem::EventMsg(EventMsg::ClientRecoveryHandled(event))
                if event.recovery_id == recovery_id
                    && event.recovery_identity.as_deref() == Some(recovery_identity)
                    && event.turn_id == tc.sub_id
        )
    }));
}

#[test]
fn resumed_rollout_requeues_only_unhandled_client_recovery() {
    let first_identity = "11111111-1111-4111-8111-111111111111";
    let second_identity = "22222222-2222-4222-8222-222222222222";
    let first_id = format!("client-recovery:{first_identity}");
    let second_id = format!("client-recovery:{second_identity}");
    let first_event = ClientRecoveryRecordedEvent {
        id: first_id.clone(),
    ...
    second_event.id = second_id.clone();
    second_event.turn_id = second_id.clone();
    second_event.recovery_identity = Some(second_identity.into());
    second_event.evidence_path = "/tmp/evidence-2".into();
    let first =
        RolloutItem::EventMsg(EventMsg::ClientRecoveryRecorded(first_event));
    let second =
        RolloutItem::EventMsg(EventMsg::ClientRecoveryRecorded(second_event));

    let pending =
        pending_client_recoveries_from_rollout(&[first.clone(), second.clone()]);
    assert_eq!(pending.len(), 2);
    assert_eq!(pending[0].1, first_id);
    assert_eq!(pending[1].1, second_id);
    assert_eq!(
        pending_client_recoveries_from_rollout(&[first.clone(), first.clone()]).len(),
        1,
        "duplicate durable Recorded facts must produce one pending wake"
    );

    let first_handled = RolloutItem::EventMsg(EventMsg::ClientRecoveryHandled(
    ...
    let after_first = pending_client_recoveries_from_rollout(&[
        first.clone(),
        second.clone(),
        first_handled.clone(),
    ]);
    assert_eq!(after_first.len(), 1);
    assert_eq!(after_first[0].1, second_id);

    let second_handled = RolloutItem::EventMsg(EventMsg::ClientRecoveryHandled(
        protocol::protocol::ClientRecoveryHandledEvent {
    ...
    assert!(
        pending_client_recoveries_from_rollout(&[
            first.clone(),
            second.clone(),
            second_handled.clone(),
        ])
            .len()
            == 1,
        "handling the second identity must not handle the first"
    );
    assert!(
        pending_client_recoveries_from_rollout(&[
            first,
        ...
        .is_empty(),
        "each modern recovery identity must be handled independently"
    );
}

...
async fn routes_approval_to_guardian_requires_guardian_reviewer() {
    ...
    assert!(!approval_service::guardian::routes_approval_to_guardian(
...
async fn guardian_review_request_layout_matches_model_visible_request_snapshot(
    ...
    let outcome = run_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        request,
        Some("Sandbox denied outbound git push to github.com.".to_string()),
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    let first_outcome = run_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        first_request,
    ...
    let second_outcome = run_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        second_request,
    ...
    let third_outcome = run_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        third_request,

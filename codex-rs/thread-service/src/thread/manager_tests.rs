...
fn fork_thread_accepts_legacy_usize_snapshot_argument() {
    ...
    let _: fn(&ThreadService, Config, std::path::PathBuf) = assert_legacy_snapshot_callsite;
}
...
"#,
    ...
    let prompt_items = crate::prompt_debug::build_prompt_input_from_session(
        Arc::clone(&thread.thread.codex.session),
        Vec::<UserInput>::new(),
    )
    .await
    .expect("prompt input");
...
async fn resume_and_fork_do_not_restore_thread_environments_from_rollout() {
    ...
    let manager = ThreadService::new_with_mcp_auth_runtime(
        &config,

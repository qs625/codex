...
use rollout_trace::ThreadStartedTraceMetadata;
use rollout_trace::ToolCallRequester;
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    dispatch_tool_via_tool_service(
        ...
        Arc::clone(&turn),
        "direct-call",
        tool_service_api::ToolName::plain("get_goal"),
    ...
    let replayed = rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
...
async fn dispatch_lifecycle_trace_records_unsupported_tool_failures() -> anyhow::Result<()> {
    ...
    let result = dispatch_tool_via_tool_service(
        ...
        turn,
        "unsupported-call",
        tool_service_api::ToolName::plain("missing_tool"),
        ToolCallSource::Direct,
        ToolPayload::Function {
            arguments: "{}".to_string(),
        },
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace = rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
        root,

...
/// Base config deserialized from ~/.morpheus/config.toml.
...
pub struct ConfigToml {
    ...
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses `$MORPHEUS_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Codex writes log files, for example `codex-tui.log`.
    /// Defaults to `$MORPHEUS_HOME/log`.
    ...
    /// directories for `.morpheus` folders. Defaults to [".git"] when unset.
    #[serde(default)]

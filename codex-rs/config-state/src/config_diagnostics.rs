//! Lightweight config diagnostics and formatting helpers.
...
#[derive(Clone, Copy)]
pub enum ConfigDiagnosticSource<'a> {
    Path(&'a Path),
    DisplayName(&'a str),
}
...
pub fn config_error_from_toml_for_source(
...
pub fn config_error_from_typed_toml_for_source<T: DeserializeOwned>(
    source: ConfigDiagnosticSource<'_>,
    contents: &str,
) -> Option<ConfigError> {
...
pub fn text_range_from_span(contents: &str, span: std::ops::Range<usize>) -> TextRange {
...
pub fn default_range() -> TextRange {
...
pub fn span_for_config_path(contents: &str, path: &SerdePath) -> Option<std::ops::Range<usize>> {
...
pub fn span_for_toml_key_path(contents: &str, path: &[String]) -> Option<std::ops::Range<usize>> {
    let doc = contents.parse::<Document<String>>().ok()?;

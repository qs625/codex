...
use crate::test_support::load_tool_suggest_plugin_ids;
...
async fn list_discoverable_plugins_for_test(
    ...
    let config = load_plugins_config(codex_home, codex_home).await;
    let (configured_plugin_ids, disabled_plugin_ids) =
        load_tool_suggest_plugin_ids(codex_home, codex_home).await;
...
"#,
    ...
    let discoverable_plugins = list_discoverable_plugins_for_test(codex_home.path())
        .await
        .unwrap();

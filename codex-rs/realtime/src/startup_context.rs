...
pub(crate) const CURRENT_THREAD_SECTION_TOKEN_BUDGET: usize = 1_200;
pub(crate) const RECENT_WORK_SECTION_TOKEN_BUDGET: usize = 2_200;
pub(crate) const WORKSPACE_SECTION_TOKEN_BUDGET: usize = 1_600;
pub(crate) const NOTES_SECTION_TOKEN_BUDGET: usize = 300;
...
pub async fn build_realtime_startup_context(
    cwd: &AbsolutePathBuf,
    current_thread_items: &[ResponseItem],
    recent_threads: &[StoredThread],
    budget_tokens: usize,
) -> Option<String> {
    let current_thread_section = build_current_thread_section(current_thread_items);

...
pub struct TurnContext {
    ...
    pub(crate) self_weak: OnceLock<Weak<TurnContext>>,
...
impl TurnContext {
    ...
    ) -> Result<Option<codex_tool_runtime_api::ResolvedExecCommandEnvironment>, FunctionCallError>
    {
        ...
        let Some(turn_environment) = environment else {
            return Ok(None);
        };
    ...
    pub(crate) fn tool_sandbox_context(&self) -> codex_tool_runtime_api::ToolSandboxContext {
        codex_tool_runtime_api::ToolSandboxContext {
            ...
            managed_network_active: self.network.is_some(),
            #[allow(deprecated)]
            cwd: self.cwd.clone(),
    ...
    pub(crate) fn tool_dispatch_telemetry(&self) -> SharedSessionTelemetry {
    ...
    pub(crate) fn base_tool_result_tags(&self) -> codex_tool_runtime_api::ToolTelemetryTags {
        vec![
            (
                "sandbox",
                codex_sandboxing_api::permission_profile_sandbox_tag(
            ...
            (
                "sandbox_policy",
                codex_sandboxing_api::permission_profile_policy_tag(
                    &self.permission_profile,
                    #[allow(deprecated)]
                    self.cwd.as_path(),
                )
    ...
    pub(crate) fn collaboration_mode_kind(&self) -> ModeKind {
    ...
    pub(crate) fn is_non_root_agent(&self) -> bool {
    ...
    pub(crate) fn next_child_spawn_depth(&self) -> i32 {
        crate::agent::next_thread_spawn_depth(&self.session_source)
    }

    pub(crate) fn agent_max_depth(&self) -> i32 {
        self.config.agent_max_depth
    }

    pub(crate) fn agent_max_threads(&self) -> Option<usize> {
        self.config.agent_max_threads
    }

    pub(crate) fn default_wait_agent_timeouts(&self) -> (i64, i64) {
        (
            self.config.multi_agent_v2.default_wait_timeout_ms,
            self.config.multi_agent_v2.max_wait_timeout_ms,
        )
    }

    pub(crate) fn default_agent_job_max_runtime_seconds(&self) -> Option<u64> {
        self.config.agent_job_max_runtime_seconds
    }
    ...
    pub(crate) fn hide_spawn_agent_metadata(&self) -> bool {
        self.config.multi_agent_v2.hide_spawn_agent_metadata
    }

    pub(crate) fn turn_environment_selections(&self) -> Vec<TurnEnvironmentSelection> {
        self.environments.to_selections()
    }
    ...
    pub(crate) fn spawn_agent_environment_selections(
        ...
        let mut environments = self.environments.to_selections();
        if let Some(cwd) = cwd {
    ...
    pub(crate) fn single_local_environment_cwd(
        &self,
    ) -> Result<AbsolutePathBuf, FunctionCallError> {
        let [turn_environment] = self.environments.turn_environments.as_slice() else {
    ...
    pub(crate) fn record_multi_agent_spawn_metric(&self, role_tag: &str) {
    ...
    pub(crate) fn build_agent_shared_config(
        &self,
        cwd: Option<AbsolutePathBuf>,
    ...
    pub(crate) fn apply_spawn_agent_runtime_overrides(
        ...
        config
            .permissions
            .approval_policy
            .set(self.approval_policy.value())
            .map_err(|err| {
                FunctionCallError::RespondToModel(format!("approval_policy is invalid: {err}"))
            })?;
        config.permissions.shell_environment_policy = self.shell_environment_policy.clone();
        config.codex_linux_sandbox_exe = self.codex_linux_sandbox_exe.clone();
        #[allow(deprecated)]
        let turn_cwd = self.cwd.clone();
        config.cwd = turn_cwd;
        config
            .permissions
            .set_permission_profile(self.permission_profile())
            .map_err(|err| {
    ...
    pub(crate) fn tool_call_mcp_elicitation_enabled(&self) -> bool {
    ...
    ) -> codex_mcp_runtime::McpToolApprovalReviewContext<'a> {
        codex_mcp_runtime::McpToolApprovalReviewContext {
    ...
    pub(crate) fn is_apply_patch_streaming_events_enabled(&self) -> bool {
        self.features.enabled(Feature::ApplyPatchStreamingEvents)
    }

    pub(crate) fn code_mode_enabled(&self) -> bool {
        self.features.enabled(Feature::CodeMode)
    }

    pub(crate) fn load_workflow_registry(&self) -> codex_workflow_api::WorkflowRegistry {
        crate::workflows::load_workflow_registry(&self.config)
    }
    ...
    pub(crate) fn mcp_skill_dependency_turn_context(
        &self,
    ) -> codex_mcp_runtime::McpSkillDependencyTurnContext<'_> {
        codex_mcp_runtime::McpSkillDependencyTurnContext {
            sub_id: &self.sub_id,
            approval_policy: self.approval_policy.value(),
            permission_profile: self.permission_profile(),
        }
    ...
    pub(crate) fn codex_app_tool_policy(
        ...
        tool_name: &str,
    ) -> codex_mcp_runtime::AppToolPolicy {
    ...
    pub(crate) fn explicit_shell_env_overrides(&self) -> HashMap<String, String> {
        self.shell_environment_policy.r#set.clone()
    }

    pub(crate) fn shell_exec_env(&self, thread_id: ThreadId) -> HashMap<String, String> {
        crate::exec_env::create_env(&self.shell_environment_policy, Some(thread_id))
    }
    ...
    #[allow(deprecated)]
    pub(crate) fn legacy_cwd(&self) -> AbsolutePathBuf {
        self.cwd.clone()
    }
    ...
    pub(crate) fn emit_memories_usage_metric(&self, kind: &str, tool: &str, success: &str) {
        self.session_telemetry.counter(
            "codex_turn_memories_usage",
            /*inc*/ 1,
            &[("kind", kind), ("tool", tool), ("success", success)],
        );
    }

    pub(crate) fn external_context_pollutes_memory_mode(&self) -> bool {
        self.config.memories.disable_on_external_context
    }

    #[allow(deprecated)]
    pub(crate) fn resolve_shell_workdir(&self, workdir: Option<String>) -> AbsolutePathBuf {
        self.resolve_path(workdir)
    }
...
impl codex_thread_api::ThreadCapability for TurnContext {
...
impl Session {
    ...
    async fn new_turn_from_configuration(
        ...
        turn_context.session = self.self_weak.get().cloned().unwrap_or_default();
        ...
        let _ = turn_context.self_weak.set(Arc::downgrade(&turn_context));

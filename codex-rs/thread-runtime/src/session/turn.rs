...
async fn run_sampling_request(
    ...
    let tool_session_capability: Arc<dyn codex_thread_api::ToolSessionCapability> =
        Arc::clone(&sess) as Arc<dyn codex_thread_api::ToolSessionCapability>;
    let tool_inputs = Arc::new(
        built_tools(
        Arc::clone(&sess),
        Arc::clone(&turn_context),
        Arc::downgrade(&tool_session_capability),
        &input,
        explicitly_enabled_connectors,
        skills_outcome,
        &cancellation_token,
    ...
    loop {
        ...
        let prompt = build_prompt(PromptBuildParams {
            input: prompt_input,
            tools: sess.services.tool_service.model_visible_specs(tool_service_request(
                &sess,
                &turn_context,
                &tool_inputs,
            )),
...
pub(crate) async fn built_tools(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    tool_session_capability: std::sync::Weak<dyn codex_thread_api::ToolSessionCapability>,
    input: &[ResponseItem],
    explicitly_enabled_connectors: &HashSet<String>,
    skills_outcome: Option<&SkillLoadOutcome>,
    cancellation_token: &CancellationToken,
...
pub(crate) fn tool_service_request<'a>(
    ...
    codex_tool_service_api::ToolSpecRequest {
        ...
        session_capability: tool_inputs.tool_session_capability.clone(),
        session: Arc::clone(sess) as Arc<dyn codex_thread_api::ToolServiceSessionRef>,
        turn: Arc::clone(turn_context) as Arc<dyn codex_thread_api::ToolServiceTurnRef>,

...
use codex_tool_types::DiscoverableToolAction;
use codex_tool_types::DiscoverableToolType;
use codex_tool_types::REQUEST_PLUGIN_INSTALL_APPROVAL_KIND_VALUE;
use codex_tool_types::RequestPluginInstallArgs;
use codex_tool_types::build_request_plugin_install_elicitation_request;
...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    assert!(
        !plugins_manager.is_configured_plugin_installed(
    ...
    assert!(plugins_manager.is_configured_plugin_installed(
        &refreshed_config.plugins_config_input(),
        "sample@openai-curated",
    ));
}

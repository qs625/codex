...
use command_service::ExecCapturePolicy;
use command_service::ExecParams;
use command_service::create_env;
use command_service::process_exec_tool_call;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<protocol::exec_output::ExecToolCallOutput> {
...
fn is_bwrap_unavailable_output(output: &protocol::exec_output::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    let dot_codex = tmpdir.path().join(".morpheus");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codex).expect("create .morpheus");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".morpheus write should be denied under bubblewrap",
    );
...
async fn sandbox_blocks_codex_symlink_replacement_attack() {
    ...
    let dot_codex = tmpdir.path().join(".morpheus");
    symlink(&decoy, &dot_codex).expect("create .morpheus symlink");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".morpheus symlink replacement should be denied",
    );
    assert_ne!(codex_output.exit_code, 0);
}
...
async fn sandbox_reports_codex_symlink_build_failure_without_panicking() {
    ...
    {
        Err(CodexErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".morpheus symlink build failure should deny: {result:?}"),
    };
...
"#,
    ...
    let mkdir_codex_output = expect_denied(
        ...
        .await,
        "child .morpheus directory creation should be denied",
    );
    assert_ne!(mkdir_codex_output.exit_code, 0);
    assert!(!subdir.join(".morpheus").exists());

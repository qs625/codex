...
use codex_core_plugins::store::PluginInstallResult;
use codex_core_plugins::store::PluginStore;
use codex_core_plugins::store::PluginStoreError;
use codex_core_plugins::store::validate_plugin_version_segment;
...
fn extract_remote_plugin_bundle_to_path(
    ...
    extract_plugin_bundle_tar_gz(&bundle_bytes, extract_dir.path())?;
    let plugin_root = find_extracted_plugin_root(extract_dir.path())?;
    let manifest = load_plugin_manifest(&plugin_root).ok_or_else(|| {

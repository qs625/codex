...
const PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME: &str = ".morpheus";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".morpheus");
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.morpheus -> <root>/decoy`, bwrap must still see
            // `<root>/.morpheus`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.morpheus` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .morpheus read-only to the agent by
    ...
    let top_level_codex = writable_root.join(PROTECTED_METADATA_PROJECT_CONFIG_PATH_NAME);

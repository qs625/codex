...
use plugin_service::installed_marketplaces::resolve_configured_marketplace_root;
use plugin_service::marketplace_add::MarketplaceAddRequest;
use plugin_service::marketplace_add::add_marketplace;
use plugin_service::marketplace_remove::MarketplaceRemoveRequest;
...
async fn run_add(args: AddMarketplaceArgs) -> Result<()> {
    ...
    let codex_home = find_codex_home().context("failed to resolve MORPHEUS_HOME")?;

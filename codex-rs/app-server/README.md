...
- `eventCommandCall` — `{id, subscriptionId, command, cwd?, label?, status, output?}` describing an EventCommand monitor. EventCommand runs a shell command in the background and treats each stdout line as a thread event; `status` is `inProgress`, `completed`, or `failed`.
- `eventCommandEvent` — `{id, subscriptionId, kind, command, cwd?, label?, line?, sequence?, exitCode?, signal?, message?, truncated, createdAt}` describing EventCommand output or terminal events. `kind` is `output`, `exited`, `cancelled`, or `failedToStart`.

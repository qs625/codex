...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$MORPHEUS_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to `$MORPHEUS_HOME/app-server-control/app-server-control.sock`
...
- `thread/list` — page through stored rollouts; supports cursor-based pagination and optional `modelProviders`, `sourceKinds`, `archived`, `cwd`, and `searchTerm` filters. Each returned `thread` includes `lifecycleStatus` (`ThreadLifecycleStatus`), using persisted lifecycle metadata when available and the legacy `notLoaded` projection when no live or durable status is known.
...
- `memory/reset` — experimental; clear the current `MORPHEUS_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `workflow/start` — create a durable workflow run, snapshot the workflow directory into `$MORPHEUS_HOME/workflow-runs/<runId>/`, and start the Node runner.
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.morpheus/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
- `eventDrivenTool` — `{id, tool, title, text}` describing an event emitted by a typed event-driven subscription, for example a schedule firing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.morpheus/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
```json
{
  ...
  "id": 26,
  "params": {
    "path": "/Users/alice/.morpheus/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.morpheus/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.morpheus/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.morpheus/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.morpheus/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },

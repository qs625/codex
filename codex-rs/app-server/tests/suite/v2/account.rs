...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![app_server_protocol::UserInput::Text {

...
async fn test_handle_token_count_event_emits_usage_and_rate_limits() -> Result<()> {
    ...
    match first {
        OutgoingMessage::AppServerNotification(ServerNotification::ThreadTokenUsageUpdated(
            payload,
        )) => {
            assert_eq!(payload.thread_id, conversation_id.to_string());
    ...
    match second {
        OutgoingMessage::AppServerNotification(ServerNotification::AccountRateLimitsUpdated(
            payload,
        )) => {

...
`codex-tool-planning` 是 `codex-core` 之外的共享工具规划 crate，负责承载可被多个
crate 复用的 model-visible tool 定义、适配、规划和内建 tool spec 构造器。
...
这个 crate 应继续承载多个消费者共享的 host-side tool 基础设施，例如：

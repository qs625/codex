...
use model_service_api::unauthenticated_auth_provider;
use protocol::account::PlanType as AccountPlanType;
use protocol::protocol::CreditsSnapshot;
use protocol::protocol::RateLimitReachedType;
...
pub fn normalize_backend_base_url(base_url: impl Into<String>) -> String {
...
pub fn rate_limits_url(base_url: &str) -> String {
    let base_url = normalize_backend_base_url(base_url);
    match PathStyle::from_base_url(&base_url) {
        PathStyle::CodexApi => format!("{base_url}/api/codex/usage"),
        PathStyle::ChatGptApi => format!("{base_url}/wham/usage"),
    }
}

pub fn send_add_credits_nudge_email_url(base_url: &str) -> String {
    let base_url = normalize_backend_base_url(base_url);
    match PathStyle::from_base_url(&base_url) {
        PathStyle::CodexApi => {
            format!("{base_url}/api/codex/accounts/send_add_credits_nudge_email")
        }
        PathStyle::ChatGptApi => {
            format!("{base_url}/wham/accounts/send_add_credits_nudge_email")
        }
    }
}

pub fn decode_rate_limit_snapshots(body: &str) -> Result<Vec<RateLimitSnapshot>> {
    let payload: RateLimitStatusPayload = serde_json::from_str(body)?;
    Ok(Client::rate_limit_snapshots_from_payload(payload))
}
...
impl Client {
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = rate_limits_url(&self.base_url);
        let req = self.http.get(&url).headers(self.headers());
        let (body, _ct) = self.exec_request(req, "GET", &url).await?;
        decode_rate_limit_snapshots(&body)
    }
    ...
    pub async fn send_add_credits_nudge_email(
        ...
        let url = send_add_credits_nudge_email_url(&self.base_url);
        let req = self
            .http
            .post(&url)
...
mod tests {
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        ...
        assert_eq!(
            send_add_credits_nudge_email_url(&transport_client.base_url),
        ...
        assert_eq!(
            send_add_credits_nudge_email_url(&chatgpt_client.base_url),

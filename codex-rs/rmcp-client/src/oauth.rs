...
//!
//! If the keyring is not available or fails, we fall back to MORPHEUS_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create MORPHEUS_HOME temp dir");
            unsafe {
                std::env::set_var("MORPHEUS_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,

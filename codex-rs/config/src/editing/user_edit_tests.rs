...
#[test]
fn set_feature_enabled_without_default_metadata_persists_false() {
    let tmp = tempdir().expect("tmpdir");
    let codex_home = tmp.path();

    ConfigEditsBuilder::new(codex_home)
        .set_feature_enabled("experimental_feature", /*enabled*/ false)
        .apply_blocking()
        .expect("persist");
...
#[test]
fn set_feature_enabled_clears_root_default_false_feature_when_metadata_is_supplied() {
    let tmp = tempdir().expect("tmpdir");
    let codex_home = tmp.path();
    std::fs::write(
...
"#,
    ...
    ConfigEditsBuilder::new(codex_home)
        .with_default_false_feature_keys(["experimental_feature"])
        .set_feature_enabled("experimental_feature", /*enabled*/ false)
        .apply_blocking()
        .expect("persist");

    let contents = std::fs::read_to_string(codex_home.join(CONFIG_TOML_FILE)).expect("read config");
    assert_eq!(contents, "[features]\n");
}
...
async fn async_builder_set_model_persists() {
    ...
    ConfigEditsBuilder::new(&codex_home)
        .set_model(Some("gpt-5.4"), Some("high".to_string()))
        .apply()
        .await
        .expect("persist");
...
"#;
    ConfigEditsBuilder::new(codex_home)
        .set_model(Some("o4-mini"), Some("low".to_string()))
        .apply_blocking()

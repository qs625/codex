//! Lightweight Agent Identity signing helpers.
//!
//! This crate intentionally owns only local task assertion signing. HTTP
//! registration, JWKS fetch, JWT verification, and key generation remain in the
//! `codex-agent-identity` runtime crate.
...
mod tests {
    ...
    fn authorization_header_for_agent_task_rejects_mismatched_runtime() {
        ...
        assert_eq!(
            error.to_string(),
            "agent task runtime other-agent does not match stored agent identity agent-123"
        );
    }
}

...
```bash
cargo run -p app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/app-server-test-client/app-server.log`
...
```bash
cargo run -p app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin app-server-test-client -- \
...
```bash
cargo run --bin app-server-test-client -- thread-resume <THREAD_ID>
```

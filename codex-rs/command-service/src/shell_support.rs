...
pub(crate) fn disable_powershell_profile_for_elevated_windows_sandbox(
    command: &[String],
    shell_type: Option<ToolUserShellType>,
    sandbox: SandboxType,
    windows_sandbox_level: WindowsSandboxLevel,
) -> Vec<String> {
    if shell_type != Some(ToolUserShellType::PowerShell)
...
pub fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if !paths_match_after_normalization(snapshot.cwd.as_path(), cwd) {
        return command.to_vec();
    }
    if command.len() < 3 || command[1].as_str() != "-lc" {
        return command.to_vec();
    }

    let snapshot_path = shell_single_quote(snapshot.path.to_string_lossy().as_ref());
    let shell_path = session_shell.shell_path.to_string_lossy().to_string();
    ...
    vec![shell_path, "-c".to_string(), rewritten_script]
}
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    let mut keys = explicit_env_overrides
        ...
        .collect::<Vec<_>>();
    keys.sort_unstable();
    build_override_exports_for_keys("__COMMAND_SERVICE_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__COMMAND_SERVICE_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__COMMAND_SERVICE_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", CODEX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__COMMAND_SERVICE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}

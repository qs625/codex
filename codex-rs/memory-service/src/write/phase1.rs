...
pub async fn run(context: Arc<MemoryStartupContext>, settings: Arc<MemoryStartupSettings>) {
    let stage_one_context: StageOneRequestContext =
        build_request_context(context.as_ref(), settings.as_ref()).await;
...
async fn run_jobs(
    ...
    futures::stream::iter(claimed_candidates.into_iter())
        .map(|claim| {
            ...
            let settings: Arc<MemoryStartupSettings> = Arc::clone(&settings);

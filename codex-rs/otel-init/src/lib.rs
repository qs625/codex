...
/// Lightweight view of the application configuration needed to initialize OTEL.
///
/// Callers should project their full runtime config into this type instead of
/// making OTEL initialization depend on the core session runtime crate.
pub struct OtelProviderConfig<'a> {
...
/// Build an OpenTelemetry provider from a lightweight config view.
///
/// Returns `None` when OTEL export is disabled.
pub fn build_provider(
    config: OtelProviderConfig<'_>,
    service_version: &str,
    ...
    OtelProvider::from(&OtelSettings {
        ...
        trace_exporter,
        metrics_exporter,
        runtime_metrics: config.runtime_metrics_enabled,
...
///
/// Keeps events that originated from the `codex_otel` module.

...
mod tests {
    ...
    #[test]
    fn constrained_normalizer_applies_on_init_and_set() {
        let mut constrained =
            Constrained::normalized(/*initial_value*/ -1, |value| value.max(0))
                .expect("normalizer should accept initial value");
        assert_eq!(constrained.value(), 0);
        constrained.set(/*value*/ -5).expect("set should pass");
        assert_eq!(constrained.value(), 0);
        constrained.set(/*value*/ 10).expect("set should pass");
        assert_eq!(constrained.value(), 10);
    }

    #[test]
    fn constrained_add_validator_composes_with_existing_validator() {
        ...
        constrained
            ...
            .expect("validator should compose");

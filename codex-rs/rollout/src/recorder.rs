...
///
/// ```ignore
/// $ jq -C . ~/.morpheus/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.morpheus/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
        ...
        let (file, deferred_log_file_info, rollout_path, meta, thread_id) = match params {
            RolloutRecorderParams::Create {
                ...
                let session_meta = SessionMeta {
                    ...
                    agent_role: root_agent_role.or_else(|| source.get_agent_role()),
                    agent_path: root_agent_path.or_else(|| source.get_agent_path().map(Into::into)),
                    source,
                    thread_source,
...
fn fill_missing_thread_item_metadata(item: &mut ThreadItem, state_item: ThreadItem) {
    ...
    if item.agent_path.is_none() {
        item.agent_path = agent_path;
    }
    if item.thread_status.is_none() {
        item.thread_status = thread_status;
    }
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.morpheus/sessions/YYYY/MM/DD path.

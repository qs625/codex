...
use crate::manager::ModelCachePolicy;
...
use crate::provider_auth_support::auth_manager_for_provider;
use crate::provider_auth_support::login_auth_manager;
use crate::provider_models_endpoint::OpenAiModelsEndpoint;
...
pub fn model_provider_info_to_api_provider(
    ...
    Ok(model_service_api::model_provider_info_to_api_provider(
        provider, auth_mode,
    ))
}
...
mod tests {
    ...
    use model_service_api::DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL;
    use model_service_api::ModelProviderAwsAuthInfo;
    use model_service_api::ProviderCapabilities;
    ...
    fn test_codex_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("model-service-test-{}", std::process::id()))
    }
    ...
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model = crate::model_info::model_info_from_slug("custom-bedrock-model");
    ...
    async fn configured_provider_uses_configured_static_catalog_when_present() {
        let custom_model = crate::model_info::model_info_from_slug("custom-provider-model");
